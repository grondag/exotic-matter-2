...
public class MeshHelper {
    ...
    private static void makeIcosahedronFace(boolean topHalf, int p1, int p2, int p3, Vec3d[] points,
            @Nullable Vec3d[] normals, IWritablePolyStream stream) {
        IMutablePolygon writer = stream.writer();
        if (normals == null) {
            if (topHalf) {
     ...
     * {@link #makePaintableBox(AxisAlignedBB, IPolygon, Consumer)} TODO: remove
     */
    ...
    public static void makePaintableBox(AxisAlignedBB box, IMutablePolygon template, Consumer<IMutablePolygon> target) {
        ...
        quad.setupFaceQuad(EnumFacing.UP, 1 - box.maxX, box.minZ, 1 - box.minX, box.maxZ, 1 - box.maxY,
                EnumFacing.SOUTH);
        target.accept(quad);
        ...
        // +X
        quad = template.claimCopy(4);
        quad.setupFaceQuad(EnumFacing.EAST, 1 - box.maxZ, box.minY, 1 - box.minZ, box.maxY, 1 - box.maxX,
                EnumFacing.UP);
        target.accept(quad);
