...
public class SimpleJoin
{
    ...
    {
    	byte j = 0;
    	if(up) j |= NeighborBlocks.FACE_FLAGS[Direction.UP.ordinal()];
    	if(down) j |= NeighborBlocks.FACE_FLAGS[Direction.DOWN.ordinal()];
    	if(east) j |= NeighborBlocks.FACE_FLAGS[Direction.EAST.ordinal()];
    	if(west) j |= NeighborBlocks.FACE_FLAGS[Direction.WEST.ordinal()];
    	if(north) j |= NeighborBlocks.FACE_FLAGS[Direction.NORTH.ordinal()];
    	if(south) j |= NeighborBlocks.FACE_FLAGS[Direction.SOUTH.ordinal()];
        this.joins = j;
    }
