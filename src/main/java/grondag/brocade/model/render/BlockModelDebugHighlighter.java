...
//        GlStateManager.enableDepth();
    ...
    private static void drawQuad(double d0, double d1, double d2, IPolygon quad, Tessellator tessellator,
        ...
        for (int i = 0; i < vertexCount; i++) {
            ...
            int zeroCount = 0;
            if (Math.abs(n.x()) < 0.0000001)
                zeroCount++;
            if (Math.abs(n.y()) < 0.0000001)
                zeroCount++;
            if (Math.abs(n.z()) < 0.0000001)
                zeroCount++;
            if (zeroCount == 2)
                return;
