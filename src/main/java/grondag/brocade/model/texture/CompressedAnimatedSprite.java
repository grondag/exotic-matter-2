...
//    public static final ConcurrentPerformanceCounter perfLoadTransfer = new ConcurrentPerformanceCounter();
    ...
    public boolean load(@Nonnull IResourceManager manager, @Nonnull ResourceLocation location,
        ...
        ExecutorCompletionService<Pair<Integer, int[][]>> runner = new ExecutorCompletionService<Pair<Integer, int[][]>>(
                loaderPool);
        ...
        while (keepGoing) {
            try {
                ResourceLocation frameLoc = new ResourceLocation(location.getNamespace(),
                        baseName.concat("_" + frameIndex + ".jpg"));
...
//                        perfLoadTransfer.endRun(start);
                    ...
                    if (this.isCompressed) {
                        this.glCompressedTextureID[frameResult.getLeft()] = TextureHelper
                                .getCompressedTexture(frameResult.getRight(), this.width);
                    } else {
                        this.rawImageData[frameResult.getLeft()] = TextureHelper
                                .getBufferedTexture(frameResult.getRight());
                    }
                    completedFrameCount++;
                }
//                perfLoadTransfer.endRun(start);
...
//                    start = perfLoadAlpha.startRun();
                    ...
                    image.getRGB(0, 0, CompressedAnimatedSprite.this.width, CompressedAnimatedSprite.this.height,
                            pixels, 0, CompressedAnimatedSprite.this.width);
...
//                    start = perfLoadMipMap.startRun();
                    ...
                    template[0] = pixels;
                    int[][] result = TextureUtil.generateMipmapData(CompressedAnimatedSprite.this.mipmapLevels,
                            CompressedAnimatedSprite.this.width, template);
        ...
        if (this.isValid && ConfigXM.RENDER.enableAnimatedTextures) {
            ...
            if (this.tickCounter >= this.ticksPerFrame) {
                try {
                    ...
                    if (this.isCompressed) {
                        TextureHelper.loadCompressedTextureFrame(this.glCompressedTextureID[frameCounter],
                                this.mipmapLevels + 1, this.originX, this.originY, this.width);
                    } else {
                        TextureHelper.uploadTextureMipmap(this.rawImageData[frameCounter], this.width, this.height,
