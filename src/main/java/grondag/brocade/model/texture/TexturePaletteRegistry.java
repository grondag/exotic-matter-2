...
public class TexturePaletteRegistry implements Iterable<ITexturePalette> {
    ...
    /**
     * Important that we have at least one texture and should have ordinal zero so
     * that it is the default value returned by modelState. Is not meant for user
     * selection. For CUT paint layer means should use same texture as base layer.
    ...
    public static final ITexturePalette NONE = addTexturePallette("none", "noise_moderate",
            new TexturePaletteSpec(Brocade.INSTANCE).withVersionCount(4).withScale(TextureScale.SINGLE)
                    .withLayout(TextureLayout.SPLIT_X_8).withRotation(RANDOM.with(ROTATE_NONE))
                    .withRenderIntent(TextureRenderIntent.BASE_ONLY).withGroups(TextureGroup.ALWAYS_HIDDEN));
    ...
    public static ITexturePalette get(int ordinal) {
        return ordinal < 0 || ordinal >= allByOrdinal.size() ? NONE
                : NullHandler.defaultIfNull(allByOrdinal.get(ordinal), NONE);
    }

    public static ITexturePalette addTexturePallette(String systemName, String textureBaseName,
            TexturePaletteSpec info) {
    ...
    public static ITexturePalette addZoomedPallete(ITexturePalette source) {
        ITexturePalette result = new TexturePallette(source.systemName() + ".zoom", source.textureBaseName(),
                new TexturePaletteSpec(source).withZoomLevel(source.zoomLevel() + 1)
    ...
    public int size() {
        return allByOrdinal.size();
    }

    public boolean isEmpty() {
        return allByOrdinal.isEmpty();
    }

    public boolean contains(Object o) {
        return allByOrdinal.contains(o);
    }

    @Override
    public Iterator<ITexturePalette> iterator() {
        return allByOrdinal.iterator();
    }

    public static List<ITexturePalette> getTexturesForSubstanceAndPaintLayer(SubstanceConfig substance,
            PaintLayer layer) {
        int searchFlags = 0;
        switch (layer) {
        case BASE:
        ...
        case LAMP:
            ...
            if (FermionConfig.BLOCKS.showHiddenTextures)
                searchFlags |= TextureGroup.HIDDEN_TILES.bitFlag;
            break;
    ...
    public static ITexturePalette addBorderRandom(IGrondagMod mod, String textureName, boolean allowTile,
            boolean renderNoBorderAsTile) {
        return addTexturePallette(textureName, textureName, new TexturePaletteSpec(mod).withVersionCount(4)
                .withScale(TextureScale.SINGLE).withLayout(TextureLayout.BORDER_13)
                .withRotation(FIXED.with(ROTATE_NONE))
                .withRenderIntent(
                        allowTile ? TextureRenderIntent.BASE_OR_OVERLAY_NO_CUTOUT : TextureRenderIntent.OVERLAY_ONLY)
                .withGroups(allowTile ? TextureGroup.STATIC_BORDERS : TextureGroup.STATIC_TILES,
                        TextureGroup.STATIC_BORDERS)
    ...
    public static ITexturePalette addBorderSingle(IGrondagMod mod, String textureName) {
        return addTexturePallette(textureName, textureName,
                new TexturePaletteSpec(mod).withVersionCount(1).withScale(TextureScale.SINGLE)
