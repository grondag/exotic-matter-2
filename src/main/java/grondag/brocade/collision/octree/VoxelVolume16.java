...
public class VoxelVolume16 {
     ...
     * would be x4).
     */
    private static void compoundNotOrMask(final long[] input, int inputIndex, final long[] result, int resultIndex,
            final long[] mask, int maskIndex) {
        inputIndex *= 4;
        resultIndex *= 4;
        maskIndex *= 4;
    ...
    private static boolean isZero(final long[] input, int inputIndex) {
        inputIndex *= 4;
        return input[inputIndex] == 0 && input[inputIndex + 1] == 0 && input[inputIndex + 2] == 0
