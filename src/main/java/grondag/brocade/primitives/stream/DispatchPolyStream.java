...
import grondag.exotic_matter.model.render.QuadListKeyBuilder;
import grondag.exotic_matter.varia.intstream.IntStreams;
import net.minecraft.util.BlockRenderLayer;
import net.minecraft.util.EnumFacing;

/**
 * Read-only stream that builds linked lists of quads based on actual face. 
 * Used for space-efficient block render dispatch.<p>
 *
 * Will have a format with links no matter how claimed
 * and will ignore any links in the stream used to create it.
 */
public class DispatchPolyStream extends AbstractPolyStream implements IReadOnlyPolyStream, Consumer<IPolygon>
{
    ...
    public final int getOcclusionHash(@Nullable  EnumFacing face)
    {
        assert isBuilt;
    ...
    private int listHeadAddress(BlockRenderLayer layer, @Nullable EnumFacing actualFace)
    {
    ...
    /**
     * Returns IPolygon#NO_LINK_OR_TAG if no polys with given layer and actual face are present.
     */
    public final int firstAddress(BlockRenderLayer layer, @Nullable EnumFacing actualFace)
    {
        assert isBuilt;
    ...
    @Override
    public void accept(@SuppressWarnings("null") IPolygon p)
    {
        assert !isBuilt;
    ...
    public void build()
    {
        ...
        while(currentAddress != IPolygon.NO_LINK_OR_TAG)
        {
            ...
            final int listAddress = listHeadAddress(p.getRenderLayer(), p.getActualFace());
