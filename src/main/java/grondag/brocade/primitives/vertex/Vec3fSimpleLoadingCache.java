...
class Vec3fSimpleLoadingCache
{
    ...
    public Vec3f get(final float x, final float y, final float z)
    {
        ...
        final long hash = ((long) Float.floatToIntBits(x))
                ^ (((long)Float.floatToIntBits(y)) << 16) 
    ...
    {        
        ...
        do
        {
            @Nullable Vec3f currentKey;       
            synchronized(writeLock)
            {
                currentKey = localState.values[position];
                if(currentKey == null)
                {
                    //write value start in case another thread tries to read it before we can write it
