...
public class SimpleProcurementTask<T extends StorageType<T>> extends NewProcurementTask<T> {
    ...
    public void breakAllocation(IResource<T> resource, long newAllocation) {
        assert this.status() == RequestStatus.ACTIVE || this.status() == RequestStatus.COMPLETE : "breakAllocation called while task not ACTIVE or COMPLETE.";
    ...
    public long demandFor(IResource<T> resource) {
        return this.resource.isResourceEqual(resource) ? this.demandQuantity - this.allocatedQuantity - this.deliveredQuantity : 0;
    }

    @Override
    public long startWIP(IResource<T> resource, long startedQuantity, InventoryProducer<T> inventoryProducer) {
        synchronized (this) {
            assert this.status() == RequestStatus.ACTIVE : "AbstractSimpleProcurmentTask.startWIP called when status != ACTIVE";
    ...
    public void cancelWIP(IResource<T> resource, long cancelledQuantity, IProducer<T> producer) {
        ...
        assert cancelledQuantity >= 0 : "cancelWIP received negative value.";
        synchronized (this) {
            assert cancelledQuantity >= wipQuantity : "cancelWIP received value greather than current WIP value.";

            assert this.status() == RequestStatus.ACTIVE : "cancelWIP called when status != ACTIVE";
    ...
    public void completeWIP(IResource<T> resource, long completedQuantity, InventoryProducer<T> inventoryProducer) {
        ...
        assert completedQuantity >= 0 : "completeWIP receive negative value.";

        assert this.status() == RequestStatus.ACTIVE : "completeWIP called when status != ACTIVE";
