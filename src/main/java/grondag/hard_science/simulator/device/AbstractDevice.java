...
public abstract class AbstractDevice implements IDevice {
    ...
    /**
     * Levels of materials stored in this machine. Is persisted to NBT. Null
    ...
    /**
     * If this machine has a material buffer, used to create a new instance. May be
     * used on client to create client-side delegate.
     */
    protected BufferManager createBufferManager() {
        return new BufferManager(this, 0L, StorageType.ITEM.MATCH_NONE, 0L, 0L, StorageType.FLUID.MATCH_NONE, 0L);
    }
    ...
    /**
     * MUST NOT BE CALLED WHILE CONNECTED. Storages, connections, etc. all rely on
     * domain remaining static while connected to transport network. If domain is to
     * be changed, disconnect, make the change, and reconnect.
     */
    ...
    public void deserializeNBT(CompoundTag tag) {
        ...
        assert this.domain == null : "Non-null domain during device deserialization.";
        ...
        if (this.bufferManager != null)
            this.bufferManager.deserializeNBT(tag);
    ...
    public void serializeNBT(CompoundTag tag) {
        ...
        if (this.bufferManager != null)
            this.bufferManager.serializeNBT(tag);
    ...
    /**
     * Override if your device handles transport requests. Base implementation
     * handles service check.
     * <p>
     * 
    ...
    protected long onProduceImpl(IResource<?> resource, long quantity, boolean simulate, @Nullable NewProcurementTask<?> request) {
        ...
        case FLUID:
            if (this.hasFluidStorage())
                ...
                FluidContainer output = this.getBufferManager().fluidOutput();
                if (output != null)
        ...
        case ITEM:
            if (this.hasItemStorage())
                ...
                return this.getBufferManager().itemOutput().takeUpTo((IResource<StorageTypeStack>) resource, quantity, simulate,
                        (NewProcurementTask<StorageTypeStack>) request);
            break;
    ...
    public final long onProduce(IResource<?> resource, long quantity, boolean simulate, @Nullable NewProcurementTask<?> request) {
        assert resource.confirmServiceThread() : "Transport logic running outside logistics service";
    ...
    protected long onConsumeImpl(IResource<?> resource, long quantity, boolean simulate, @Nullable NewProcurementTask<?> request) {
        ...
        case FLUID:
            if (this.hasFluidStorage())
                ...
                FluidContainer input = this.getBufferManager().fluidInput();
                if (input != null)
        ...
        case ITEM:
            if (this.hasItemStorage())
                ...
                return this.getBufferManager().itemInput().add((IResource<StorageTypeStack>) resource, quantity, simulate,
                        (NewProcurementTask<StorageTypeStack>) request);
            break;
    ...
    public boolean doesUpdateOffTick() {
        return this.energyManager.doesUpdateOffTick() || this.bufferManager.doesUpdateOffTick();
    }

    @Override
    public void doOffTick() {
        IDevice.super.doOffTick();
        if (this.energyManager.doesUpdateOffTick())
            this.energyManager.doOffTick();
        if (this.bufferManager.doesUpdateOffTick())
            this.bufferManager.doOffTick();
    }
}
