...
public interface IDevice extends IIdentified, ILocated, IDomainMember, IReadWriteNBT, ISimulationTickable {
    public default boolean doesPersist() {
        return true;
    }

    /**
     * Set to true at the end of {@link #onConnect()} and set to false at the end of
     * {@link #onDisconnect()}.
     */
     ...
     */
    @Nullable
    public default IDeviceBlockManager blockManager() {
        return null;
    }

    public default boolean hasBlockManager() {
    ...
    /**
     * Null if this device has no transport facilities. If null, implies device has
     * no transport nodes.
     */
    @Nullable
    public default ITransportManager<?> tranportManager(StorageType<?> storageType) {
        return null;
    }

    public default boolean hasTransportManager(StorageType<?> storageType) {
        return this.tranportManager(storageType) != null;
    }

    @SuppressWarnings("unchecked")
    public default ITransportManager<StorageTypePower> powerTransport() {
        return (ITransportManager<StorageTypePower>) this.tranportManager(StorageType.POWER);
    }

    @SuppressWarnings("unchecked")
    public default ITransportManager<StorageTypeStack> itemTransport() {
        return (ITransportManager<StorageTypeStack>) this.tranportManager(StorageType.ITEM);
    }

    @SuppressWarnings("unchecked")
    public default ITransportManager<StorageTypeFluid> fluidTransport() {
        return (ITransportManager<StorageTypeFluid>) this.tranportManager(StorageType.FLUID);
    }

    /**
     * Signal that device should perform internal initialization and register device
     * blocks via {@link DeviceManager#addDeviceBlock}. Device should already be
     * added to device manager when this is called.
     * 
     * Called exactly once either by ...<br>
     * 1) Device Manager after all simulation components are deserialized.<br>
     * ...or...<br>
     * 2) Block placement logic after domain /location are set and and
     * deserialization from stack (if applies) is complete.
     * <p>
     */
    ...
    /**
     * Called by transport system for this device to handle outbound transport
     * requests. SHOULD ONLY BE CALLED FROM LOGISTICS SERVICE to ensure consistency
     * of results.
     */
    public long onProduce(IResource<?> resource, long quantity, boolean simulate, @Nullable NewProcurementTask<?> request);

    /**
     * Called by transport system for this device to handle inbound transport
     * requests SHOULD ONLY BE CALLED FROM LOGISTICS SERVICE to ensure consistency
     * of results.
     */
    ...
    /**
     * Called after ports on this device are attached or detached to notify
     * transport manager to update transport addressability for this device. Has no
     * effect if this device lacks transport facilities for the given storage type.
     * 
     ...
     */
    @Nullable
    public default FluidContainer fluidStorage() {
        return null;
    }
     ...
     */
    public default boolean hasFluidStorage() {
        return this.fluidStorage() != null;
    }
     ...
     */
    @Nullable
    public default ItemContainer itemStorage() {
        return null;
    }
     ...
     */
    public default boolean hasItemStorage() {
     ...
     */
    public default boolean hasChannel() {
        return this.getChannel() != CHANNEL_UNSUPPORTED;
    }

    public static final int CHANNEL_UNSUPPORTED = -1;

    /**
     * Configured transport channel for item & power connection segregation. Will
     * return {@link #CHANNEL_UNSUPPORTED} if not supported. Is generally the block
     * meta/species. Default implementation is no channel support.
     */
    public default int getChannel() {
