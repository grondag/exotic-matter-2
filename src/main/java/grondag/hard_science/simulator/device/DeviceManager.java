...
public class DeviceManager extends SimulationTopNode implements ISimulationTickable {
    ...
    /**
     * A bit ugly but covenient. Set to null when any instance is created, retrieved
     * lazily from Simulator.
     */
    ...
    /**
     * Records device for access and persistence.
     * </p>
     * Note that adding a device does NOT connect it. That is done either after
     * deserialization (for existing devices) or after block placement (for new
     * devices)
     */
    ...
    private final Int2ObjectOpenHashMap<IDevice> devices = new Int2ObjectOpenHashMap<IDevice>();
    ...
    private final PerformanceCounter perfCounterOffTick = PerformanceCounter.create(Configurator.MACHINES.enablePerformanceLogging,
            "Machine off-stick update processing", perfCollectorOffTick);
    private final PerformanceCounter perfCounterOnTick = PerformanceCounter.create(Configurator.MACHINES.enablePerformanceLogging,
            "Machine on-stick update processing", perfCollectorOnTick);
    ...
    public void addDeviceInconveniently(IDevice device) {
        ...
        assert this.devices.put(device.getId(), device) == null : "Duplicate device registration.";
    ...
    public void doOnTick() {
        ...
        if (devs == null) {
            devs = devices.size() == 0 ? new IDevice[0] : devices.values().stream().filter(d -> d.doesUpdateOnTick()).toArray(IDevice[]::new);
    ...
    public void doOffTick() {
        ...
        if (devs == null) {
            devs = devices.size() == 0 ? new IDevice[0] : devices.values().stream().filter(d -> d.doesUpdateOffTick()).toArray(IDevice[]::new);
