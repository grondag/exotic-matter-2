...
/**
 * Basic combined device block & block manager implementation for single-block
 * machines.
 */
public class SimpleBlockHandler implements IDeviceBlock, IDeviceBlockManager, IDeviceComponent {
    ...
    /**
     * Sets up ports and parents. Assumes device block position is given by device
     * location. Thus should not be called until device has a location.
     * <p>
     * 
     * Channel will be used as the channel ID for any block or item ports that rely
     * on it for carrier segregation. (Fluid ports segregate using channels derived
     * from fluid carried.)
     * <p>
     * 
     * PortLayout should already be transformed (if necessary) so that port facings
     * are actual, in-world facings instead of nominal facing of the port layout for
     * this device.
     */
    ...
    protected void onAdded() {
        ...
        if (this.itemPortManager.isEmpty() && this.powerPortManager.isEmpty() && this.fluidPortManager.isEmpty())
            return;
    ...
    private <T extends StorageType<T>> void doConnect(@Nullable BlockPortManager<T> myPortManager, LogisticsService<T> service,
            ImmutableList<Pair<IDeviceBlock, EnumFacing>> neighbors) {
        if (myPortManager.isEmpty())
            return;
        ...
        service.executor.execute(() -> {
            for (Pair<IDeviceBlock, EnumFacing> n : neighbors) {
                ...
                for (Port<T> myPort : myPortManager.getPorts(face)) {
                    ...
                    for (Port<T> matePort : n.getLeft().getConnectablePorts(myPort, face.getOpposite())) {
                        if (service.connect(myPort, matePort))
                            break;
                    }
                }
            }
    ...
    public @Nullable <T extends StorageType<T>> Iterable<Port<T>> getConnectablePorts(@Nonnull Port<T> fromPort, EnumFacing actualFace) {
        return this.portManager(fromPort.storageType()).getConnectablePorts(fromPort, actualFace);
    }
    ...
    private <T extends StorageType<T>> void doDisconnect(@Nullable BlockPortManager<T> portManager, LogisticsService<T> service) {
        if (portManager.isEmpty())
            return;
