...
public class ItemResource extends AbstractResource<StorageType.StorageTypeStack> {
    ...
    public static ItemResource fromStack(ItemStack stack) {
        if (stack == null || stack.isEmpty())
            return (ItemResource) StorageType.ITEM.emptyResource;
    ...
    public ItemStack stackWithQuantity(int quantity) {
        ...
        if (this.tag != null)
            result.setTagCompound(this.tag);
        return result;
    }
    ...
    public boolean isStackEqual(ItemStack stack) {
        if (stack == null || stack.isEmpty())
            return this == StorageType.ITEM.emptyResource;

        return stack.getItem() == this.item && stack.getMetadata() == this.meta && Objects.equal(stack.getTagCompound(), this.tag)
                && Objects.equal(ItemHelper.itemCapsNBT(stack), this.caps);
    }

    @Override
    public String displayName() {
        return this.stack == null ? this.sampleItemStack().getDisplayName() : this.stack.getDisplayName();
    }
    ...
    public boolean isResourceEqual(@Nullable IResource<?> other) {
        ...
        if (other instanceof ItemResource) {
            ...
            boolean result = resource.item == this.item && resource.meta == this.meta && Objects.equal(resource.tag, this.tag)
