...
/**
 * Used for client representation of item stacks. Test match with server-side
 * resources using the handle.
 * 
 * Includes a quantityIn directly instead of in a sub class because uses cases
 * for bulkResource delegates typically require it.
 */
public class ItemResourceDelegate implements IMessagePlus {
    public static final ItemResourceDelegate EMPTY = new ItemResourceDelegate(-1, ItemResource.fromStack(ItemStack.EMPTY), 0);
    ...
    /**
     * Delegate equality test is by handle instead of by instance and does include
     * quantityIn.
     */
    @Override
    public boolean equals(@Nullable Object other) {
        return this == other || (other != null && other.getClass() == this.getClass()
                && (((ItemResourceDelegate) other).handle() == this.handle() && ((ItemResourceDelegate) other).quantity == this.quantity));
    }
