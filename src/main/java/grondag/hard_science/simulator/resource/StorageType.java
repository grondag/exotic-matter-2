...
/**
 * Set up like an enum but using classes to enable generic-based type safety for
 * bulkResource classes. Probably a better way to do this but I don't think a
 * regular enum will do.
 */
public abstract class StorageType<T extends StorageType<T>> {
    ...
    private StorageType(EnumStorageType enumType, IResource<T> emptyResource) {
        ...
        this.MATCH_ANY = new IResourcePredicate<T>() {
            @Override
            public boolean test(@Nullable IResource<T> t) {
                return true;
            }
        };
    ...
    /**
     * Units per tick throughput for the given level on storage transport networks
     * of this storage type. Channel only matter for fluid networks, which have
     * fixed channels for each type of transportable fluid. For other storage types
    ...
    public static <V extends StorageType<V>> IResource<V> fromNBTWithType(NBTTagCompound tag) {
        StorageType<?> sType = StorageType.fromEnum(Useful.safeEnumFromTag(tag, NBT_RESOURCE_TYPE, EnumStorageType.ITEM));
        return (IResource<V>) sType.fromNBT(tag);
    }

    public static <V extends StorageType<V>> String toCSVWithType(IResource<V> resource) {
        return resource.storageType().enumType.name() + "," + resource.storageType().toCSV(resource);
    }
    ...
    public static IResource<?> fromCSVWithType(String csv) {
        ...
        try {
            ...
            return StorageType.fromEnum(sType).fromCSV(csv.substring(split[0].length() + 1));
        } catch (Exception e) {
            return null;
        }
    }
    ...
    /**
     * Materials stored as item stacks. AbstractStorage managers for other storage
     * types that can be encapsulated as item stacks will use the item stack storage
     * manager as a subsystem.
     */
    ...
    public static class StorageTypeStack extends StorageType<StorageTypeStack> {
        private StorageTypeStack() {
            super(EnumStorageType.ITEM, new ItemResource(ItemStack.EMPTY.getItem(), ItemStack.EMPTY.getMetadata(), null, null));
        }
        ...
        /**
         * Note that this expects to get an ItemStack NBT, which is what ItemResource
        ...
        public String toCSV(IResource<StorageTypeStack> resource) {
            ...
            String result = stack.getItem().getRegistryName().toString() + "," + stack.getMetadata();
    ...
    /**
     * Must be stored in a battery. Note that fuel is not counted as power because
     * making power from fuel is a non-trivial production step.
     */
