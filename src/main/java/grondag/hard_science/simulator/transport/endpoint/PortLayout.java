...
/**
 * Specifies all ports present on the faces of a block and acts as factory for
 * port instances at a specific BlockPos.
 * <p>
 * 
 * Semantics for orientation of port faces in construction methods are the same
 * used within model state and mesh generation: Y axis/UP is the default "top"
 * of the machine and north face is the default "front."
 * <p>
 * 
 * Transformed wrapper uses model state of the block as placed in world to
 * derive actual channel and facing of the ports. When serialized, the
 * transformed wrapper retains both the underlying layout and this
 * transformation.
 * <p>
 * 
 * See {@link Transform#getFaceMap(ISuperModelState)} for more information on
 * model/block orientation.
 * <p>
 * 
 * Block Device implementations should serialize port layout (including channel
 * and orientation) because layout and model state are typically provided by
 * block state / tile entity and will not be available to the device simulation
...
public class PortLayout extends IForgeRegistryEntry.Impl<PortLayout> implements IPortLayout {
    ...
    public PortLayout(String name, PortFace up, PortFace down, PortFace east, PortFace west, PortFace north, PortFace south) {
    ...
    public static IPortLayout fromNBT(NBTTagCompound nbt) {
        ...
        PortLayout result = ModRegistries.portLayoutRegistry.getValue(new ResourceLocation(nbt.getString(NBT_PORT_LAYOUT_NAME)));
