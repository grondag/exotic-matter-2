...
public class CarrierPortGroup<T extends StorageType<T>> implements Iterable<Port<T>>, ITypedStorage<T> {
    ...
    private SimpleUnorderedArrayList<Port<T>> ports = new SimpleUnorderedArrayList<Port<T>>();

    /**
     * Counts how many ports in this group are attached to the internal carrier. If
     * drops to zero, then void our reference to the internal carrier so that it
     * doesn't get used/retained improperly. Will include both bridge and carrier
     * ports.
     */
    private int carrierPortCount = 0;

    public CarrierPortGroup(IDevice device, T storageType, CarrierLevel carrierLevel) {
        this.device = device;
    ...
    public void setConfiguredChannel(int channel) {
        ...
        assert !this.carrierLevel.isTop() : "Attempt to configure channel on top-level transport device";
    ...
    public Port<T> createPort(boolean isBridge, PortConnector connector, BlockPos pos, EnumFacing face) {
        synchronized (this) {
            ...
            CarrierPortState result = new CarrierPortState(isBridge ? PortFunction.BRIDGE : PortFunction.CARRIER, connector, pos, face);
    ...
    /**
     * The number of carrier ports that are attached. If zero, then
     * {@link #internalCircuit} will be null.
     */
    ...
    public void removePort(Port<T> port) {
        synchronized (this) {
            ...
            assert this.ports.contains(port) : "Removal request for non-existant port";
    ...
    private class CarrierPortState extends Port<T> {
        ...
        public int getChannel() {
            return this.function() == PortFunction.CARRIER || this.mode == PortMode.CARRIER ? CarrierPortGroup.this.carrierChannel : super.getChannel();
        }
        ...
        /**
         * For carrier ports and bridge ports in carrier mode, will *always* return
         * internal carrier if it exists, even if this specific port is not attached.
         * <p>
         * 
         * {@inheritDoc}
         */
        ...
        public Carrier<T> externalCircuit() {
            ...
            case BRIDGE:
                ...
                assert (externalCircuit == null && internalCircuit == null)
                        || externalCircuit != internalCircuit : "External circuit matches internal circuit on bridge port";
                return externalCircuit;

            case CARRIER:
                // note we are checking raw value for external here due to override logic
                assert externalCircuit == null || externalCircuit == internalCircuit : "Mismatched external circuit on carrier port";
                return internalCircuit;
            ...
            case NO_CONNECTION_STORAGE_TYPE:
                assert externalCircuit == null : "Non-null external circuit on disconnected port";
                return null;
        ...
        /**
         * For carrier ports, externalCircuit must match existing internal circuit if
         * internal circuit is non-null.
         * 
         * For bridge ports, behaves same as direct port and also creates or joins an
         * internal carrier. (But no conflict with internal is possible because external
         ...
         */
        @Override
        public void attach(@Nonnull Carrier<T> externalCircuit, @Nonnull Port<T> mate) {
            assert confirmServiceThread() : "Transport logic running outside transport thread";

            assert internalCircuit == null ? carrierPortCount == 0 : carrierPortCount > 0 : "Inconsistent carrier count on carrier port pre-attach";

            switch (this.mode) {
            case BRIDGE:

                if (Configurator.logTransportNetwork)
                    HardScience.INSTANCE.info("CarrierPortGroup.attach %s: using bridge mode logic", this.device().machineName());
                ...
                if (internalCircuit == null) {
                    ...
                    if (Configurator.logTransportNetwork)
                        HardScience.INSTANCE.info("CarrierPortGroup.attach %s: created new internal circuit %d", this.device().machineName(),
            ...
            case CARRIER:
                ...
                if (Configurator.logTransportNetwork)
                    HardScience.INSTANCE.info("CarrierPortGroup.attach %s: using carrier mode logic", this.device().machineName());
            ...
            assert internalCircuit != null : "Missing internal carrier on carrier group post-attach.";

            assert carrierPortCount > 0 : "Inconsistent carrier count (zero) on carrier group post-attach";
        }

        /**
         * For carrier ports, will drop reference to internal circuit when last carrier
         * port is detached.
         * <p>
         * 
         * {@inheritDoc}
        ...
        public void detach() {
            ...
            assert internalCircuit != null : "Missing internal carrier on carrier port pre-detach.";

            assert carrierPortCount > 0 : "Inconsistent carrier count on carrier port pre-detach.";
        ...
        /**
         * This will be called multiple times for the same port group if multiple ports
         * are attached to the same carrier. However because all the ports are (in
         * concept) attached to the same physical carrier this can create a temporary
         * situation where carrier ports external carrier are different from the
         * internal carrier.
         * <p>
         * 
         * Normally this is prevented by only allowing carrier ports to connect,
         * one-at-a-time, if and only of the external carrier matches the internal
         * carrier.
         * <p>
         * 
         * We could have prevented this by forcing ports to disconnect when there is a
         * merge or split event, but such events are frequent, especially on world load,
         * and it could be a performance risk for large, complex networks. And it
         * wouldn't necessarily be simple, because we'd have to disconnect and reconnect
         * all involved <em>device blocks</em> (not ports) in a specific sequence that
         * would resolve the merge/split event that is currently handled by a swap.
         * <p>
         * 
         * So to prevent inconsistency within this device, we update the internal
         * carrier and <em>all ports</em> on the device. If method is called multiple
         * times because there are multiple ports, subsequent calls will be ignored
         * because the swapped circuit will no longer be present in the device.
         * <p>
         * 
         * This does not prevent temporary inconsistency of carrier circuits with port
         * <em>mates</em> but there is nothing we can do about that here.
         * <p>
         * 
         * {@inheritDoc}
         */
        ...
        public void swapCircuit(@Nonnull Carrier<T> oldCircuit, @Nonnull Carrier<T> newCircuit) {
            ...
            if (internalCircuit == oldCircuit) {
                if (Configurator.logTransportNetwork)
                    HardScience.INSTANCE.info("CarrierPortGroup.swapCircuit %s: replacing internal circuit %d with new circuit %d", this.device().machineName(),
                    ...
                    if (port.externalCircuit == oldCircuit) {
                        if (Configurator.logTransportNetwork)
                            HardScience.INSTANCE.info("CarrierPortGroup.swapCircuit %s: replacing external circuit %d with new circuit %d",
                                    this.device().machineName(), oldCircuit.carrierAddress(), newCircuit.carrierAddress());
    ...
    /**
     * Sets the channel to be used for internal carrier and all ports operating in
     * carrier mode.
     * <p>
     * 
