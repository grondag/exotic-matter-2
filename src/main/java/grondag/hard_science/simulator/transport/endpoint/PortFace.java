...
/**
 * Immutable set of ports on a single face of a block. Instances returned with
 * the same set of ports are guaranteed to be the same instance and can thus be
...
public class PortFace extends ForwardingSet<PortDescription<?>> {
    ...
    /**
     * Maps device channels to other instances. Will be null if this port faces has
     * no device channel ports. See {@link #withDeviceChannel(int)}
     */
    ...
    /**
     * Caches the results of {@link #couldConnectWith(PortFace)} There are certainly
     * more space-efficient ways to do this, but this is simple and fast.
     */
    ...
    /**
     * If this port face contains power or item ports that are keyed to a device
     * channel, returns the port face that results from setting those ports to a
     * specific channel. Power/Item device channels equate to species, and range
     * from 0-15.
     * <p>
     * 
     * If this port face does not contain any ports keyed to the device channel (in
     * other words, represents a face where those ports have already been set to the
     * device channel) then returns self.
     * <p>
     * 
     * Instances returned from this method will be the same instance that would be
     * returned from {@link #find(PortDescription...)}.
     */
    public PortFace withDeviceChannel(int deviceChannel) {
        if (withDeviceChannel == null)
            return this;
     ...
     * connect with other face provided, assuming the faces are adjacent.
     */
    ...
    private boolean couldConnectWithImpl(PortFace other) {
        ...
        for (PortDescription<?> fromPort : this) {
            for (PortDescription<?> toPort : other) {
                if (fromPort.couldAttach(toPort))
                    return true;
            }
        }
        return false;
