...
/**
 * Immutable description of port that always returns the same instance for the
 * same values. Also includes a unique transient identifier.
 * <p>
 * 
...
public class PortDescription<T extends StorageType<T>> implements IPortDescription<T> {
    private static final BitPacker64<PortDescription<?>> PACKER = new BitPacker64<PortDescription<?>>(p -> p.id, (p, i) -> {
    ...
    public static <V extends StorageType<V>> PortDescription<V> find(V storageType, CarrierLevel level, PortFunction function, PortConnector connector,
            int channel) {
        int id = (int) (STORAGE_TYPE.getBits(storageType.enumType) | LEVEL.getBits(level) | FUNCTION.getBits(function) | CONNECTOR.getBits(connector)
     ...
     */
    private PortDescription(int id, T storageType, CarrierLevel level, PortFunction function, PortConnector connector, int channel) {
        this.id = id;
    ...
    /**
     * Transient unique ID for this combination of port characteristic. NOT
     * guaranteed to be consistent across sessions/worlds because fluid IDs can
     * change if other mods are added/removed/changed.
     */
    ...
    /**
     * Returns this port with channel set to the given channel. Instances returned
     * from this method will be the same instance that would be returned from
     * {@link #find(StorageType, CarrierLevel, PortFunction, PortConnector, int)}
     */
    public PortDescription<?> withChannel(int channel) {
        return channel == this.channel ? this : find(this.storageType, this.level, this.function, connector, channel);
    }
}
