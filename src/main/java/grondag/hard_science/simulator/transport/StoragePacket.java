...
/**
 * Describes bulkResource transmission within a connected, wireless or drone
 * network. If resources must cross over network boundaries will require
 * multiple packets.
 * 
 * @see IItinerary.
 */
public class StoragePacket<T extends StorageType<T>> {
    ...
    /**
     * Contains intermediate node-to-node vectors between {@link #fromNode} and
     * {@link #toNode}. Will be empty if there is a direct route.
     */
    private final Object2LongOpenHashMap<Carrier<T>> links = new Object2LongOpenHashMap<Carrier<T>>();
    ...
    /**
     * Standardized cost of transporting this packet. Typically based on size of the
     * payload. Used by lower transport layers to compute cost of the transport on
     * specific media.
     */
    ...
    /**
     * Updates the accumulated total cost for transporting this packet, by transport
     * media. Tracked for each link separately so that can refund costs incurred at
     * each link if the transport cannot be completed.
     */
...
//    }
    ...
    /**
     * Accumulated cost of transporting this packet. Will not be final until packet
