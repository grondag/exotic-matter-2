...
/**
 * Describes one half of a route - path from a starting circuit to an ending
 * circuit. End circuit is always the highest level circuit if more than one
 * circuit. To form a route, end circuits of two legs must be the same.
 */
public abstract class Leg<T extends StorageType<T>> {
    /**
     * Lowest-level circuit in the leg. Will be same as {@link #end()} for direct
     * routes.
     */
    public abstract Carrier<T> start();

    /**
     * Highest-level circuit in the leg. Will be same as {@link #start()} for direct
     * routes.
     */
    public abstract Carrier<T> end();

    /**
     * If leg has three circuits, the circuit between {@link #start()} and
     * {@link #end()}. Null otherwise.
     */
    @Nullable
    public abstract Carrier<T> inner();

    /**
     * Number of circuits in the leg. Possible values are 1, 2 or 3.
     */
    public abstract int size();

    /**
     * Returns a new leg with end circuit as the given circuit. Only valid if size <
     * 3 and provided circuit is one level higher than current end.
     */
    ...
    private static class DoubleLeg<T extends StorageType<T>> extends Leg<T> {
        ...
        private DoubleLeg(Carrier<T> start, Carrier<T> end) {
            ...
            assert start.level().ordinal() < end.level().ordinal() : "Circuit level mismatch in leg constructor.";

            assert start.storageType() == end.storageType() : "Circuit type mismatch in leg constructor.";
        }
    ...
    private static class TripleLeg<T extends StorageType<T>> extends DoubleLeg<T> {
        ...
        private TripleLeg(Carrier<T> start, Carrier<T> inner, Carrier<T> end) {
            ...
            assert start.level() == inner.level().below() && inner.level() == end.level().below() : "Circuit level mismatch in leg constructor.";

            assert start.storageType() == inner.storageType() && inner.storageType() == end.storageType() : "Circuit type mismatch in leg constructor.";
        }
        ...
        @Override
        public String toString() {
            return String.format("%d.%d.%d", this.start.carrierAddress(), this.inner.carrierAddress(), this.end.carrierAddress());
        }
    }
}
