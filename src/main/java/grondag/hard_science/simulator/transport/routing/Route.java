...
public class Route<T extends StorageType<T>> {
    /**
     * List of all circuits in the route. First circuit will be first in the list
    ...
    /**
     * Checks assertions in dev environment. Basic premise is that circuits should
     * not appear in both legs. Relies on legs to ensure that circuits in each leg
     * are distinct. (Legs do this by ensuring adjacent circuits are one level
     * apart.)
     */
    private boolean isValid() {
        ...
        if (size == 2) {
            ...
            return this.circuits.get(0) != this.circuits.get(3) && this.circuits.get(0) != this.circuits.get(2) && this.circuits.get(1) != this.circuits.get(3);
        } else {
            return size == 5 && this.circuits.get(0) != this.circuits.get(4) && this.circuits.get(1) != this.circuits.get(3);
        }
    }

    /**
     * The endpoint circuit in the route with the lower carrier address. Will be
     * same as {@link #last()} in a direct route.
     */
    ...
    /**
     * The endpoint circuit in the route with the higher carrier address. Will be
     * same as {@link #first()} in a direct route.
     */
