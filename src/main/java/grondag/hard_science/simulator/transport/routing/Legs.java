...
public class Legs<T extends StorageType<T>> {
    ...
    /**
     * Max circuit bridge version for all included circuits at time of create. If
     * any included circuit has a version higher than this, then this information is
     * no longer valid. See {@link Carrier#bridgeVersion()}.
     */
    ...
    /**
     * Unique Top-level circuits across all legs. For two devices to be connected,
     * they must share at least one island. Allows for connectivity checking without
        ...
        if (forCircuit.parents().isEmpty()) {
            ...
            while (!workList.isEmpty()) {
                ...
                for (Carrier<T> c : workLeg.end().parents()) {
                    ...
                    if (workLeg != firstLeg && forCircuit.parents().contains(c))
                        continue;
    ...
    /**
     * Combines legs from multiple circuits - used to represent information for a
     * device attached to more than one circuit.
     */
    ...
    public boolean isCurrent() {
        ...
        for (Carrier<T> c : this.circuits) {
            if (c.bridgeVersion() > this.maxBridgeVersion)
                return false;
        }
        return true;
    }
    ...
    /**
     * All legs originating from a circuit or device (for compounded Legs). This is
     * a list of lists. The top level list has an entry for every unique end circuit
     * for which there is a leg, and is sorted by carrier level (lowest first) and
     * then by the address of the end circuit.
     * <p>
     * 
     * The top-level list may be empty, but any lists within the list are guaranteed
     * to have at least one entry.
     * <p>
     * 
     * This structure allows for fast pair-wise iterations through two leg lists
    ...
    private static class LegBuilder<T extends StorageType<T>> {
        ...
        private HashMap<Carrier<T>, ImmutableList.Builder<Leg<T>>> legBuilders = new HashMap<Carrier<T>, ImmutableList.Builder<Leg<T>>>();
        ...
        protected ImmutableList<ImmutableList<Leg<T>>> build() {
            ...
            legBuilders.entrySet().stream().sorted(new Comparator<Map.Entry<Carrier<T>, ImmutableList.Builder<Leg<T>>>>() {
                @Override
                public int compare(@Nullable Entry<Carrier<T>, Builder<Leg<T>>> o1, @Nullable Entry<Carrier<T>, Builder<Leg<T>>> o2) {
                    ...
                    return ComparisonChain.start().compare(c1.level().ordinal(), c2.level().ordinal()).compare(c1.carrierAddress(), c2.carrierAddress())
                            .result();
                }
