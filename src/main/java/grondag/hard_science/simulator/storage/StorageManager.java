...
/**
 * Responsibilities:
 * <li>Tracking the location of all resources for a storage type within a
 * domain.
 ...
 * <li>Notifies listeners when total storage changes</li>
 * <p>
 *
 * Not responsible for optimizing storage.
 */
public class StorageManager<T extends StorageType<T>> implements ITypedStorage<T>, IDomainMember, ISizedContainer, IStorageAccess<T>, IDomainCapability {
     ...
     */
    protected Key1List<StorageResourceManager<T>, IResource<T>> slots = new Key1List.Builder<StorageResourceManager<T>, IResource<T>>()
            .withPrimaryKey1Map(StorageResourceManager::resource).build();
    ...
    public ImmutableList<IResourceContainer<T>> getLocations(IResource<T> resource) {
        ...
        if (summary == null || summary.quantityStored() == 0)
            return ImmutableList.of();

        return summary.getLocations(resource).stream().sorted((StorageWithQuantity<T> a, StorageWithQuantity<T> b) -> Long.compare(a.quantity, b.quantity))
                .map(p -> p.storage).collect(ImmutableList.toImmutableList());
    }
    ...
    private void addStoreImpl(IResourceContainer<T> store) {
        assert !stores.contains(store) : "Storage manager received request to add store it already has.";
    ...
    private void removeStoreImpl(IResourceContainer<T> store) {
        assert stores.contains(store) : "Storage manager received request to remove store it doesn't have.";
    ...
    /**
     * Like {@link #getQuantityAvailable(IResource)} but does not have to be called
     * on service thread. Result is not reliable for transport planning purposes.
     */
    ...
    /**
     * Like {@link #findQuantityAvailable(Predicate)} but can be called from any
     * thread. Result may not be fully consistent and should not be used for
     * transport planning.
     */
    ...
    /**
     * Returns a list of all stores that have resources matching the predicate with
     * quantityIn included.
     */
    ...
    /**
     * Returns a list of stores that have the given resource available and reachable
     * from the given device, in order of preference for extract. (Generally stores
     * that can be emptied are preferred.)
     * <p>
     */
    public ImmutableList<IResourceContainer<T>> findSourcesFor(IResource<T> resource, IDevice reachableFrom) {
        ...
        return summary.getLocations(resource).stream().filter(s -> {
            return service.areDevicesConnected(s.storage.device(), reachableFrom, resource);
        }).sorted((StorageWithQuantity<T> a, StorageWithQuantity<T> b) -> ComparisonChain.start()
                // pull from public output buffers before
                // pulling from public storage
                .compare(b.storage.containerUsage().ordinal(), a.storage.containerUsage().ordinal())

                // pull from smaller stores first,
                // to favor having fewer containers
                .compare(a.quantity, b.quantity).result()).map(p -> p.storage).collect(ImmutableList.toImmutableList());
    }
    ...
    /**
     * Called by storage instances, or by self when a storage is removed. If request
     * is non-null, then the amount taken reduces any allocation to that request.
     */
    public synchronized void notifyTaken(IResourceContainer<T> storage, IResource<T> resource, long taken, @Nullable NewProcurementTask<T> request) {
        ...
        assert summary != null : "Storage manager encounted missing resource on resource removal.";
        ...
        assert used >= 0 : "Storage manager encounted negative inventory level.";
    ...
    /**
     * If request is non-null, then the amount added is immediately allocated to
     * that request.
     */
    public synchronized void notifyAdded(IResourceContainer<T> storage, IResource<T> resource, long added, @Nullable NewProcurementTask<T> request) {
        ...
        assert this.used <= this.capacity : "Storage manager usage greater than total storage capacity.";
    }
    ...
    public synchronized void notifyCapacityChanged(long delta) {
        ...
        assert this.capacity >= 0 : "Storage manager encounted negative capacity level.";
    }
     ...
     * and will not set allocation so that total allocated is more the total
     * available. Provides no notification to the request.
     */
    public long setAllocation(IResource<T> resource, NewProcurementTask<T> request, long requestedAllocation) {
        assert this.confirmServiceThread() : "Storage manager access outside service thread.";

        assert requestedAllocation >= 0 : "AbstractStorageManager.setAllocation got negative allocation request.";
    ...
    /**
     * Adds delta to the allocation of this resource for the given request. Return
     * value is the quantityIn removed or added, which could be different than
     * amount requested if not enough is available or would reduce allocation below
     * 0. Total quantityIn allocated can be different from return value if request
     * already had an allocation. Provides no notification to the request.
     */
    public long changeAllocation(IResource<T> resource, long quantityRequested, NewProcurementTask<T> request) {
        assert this.confirmServiceThread() : "Storage manager access outside service thread.";
    ...
    public void setDirty() {
        // NOOP - storage state is saved with the devices
        // manager state reconstructed a run time // NOOP - storage state is saved with
        // the devices
    }
