...
public class PowerContainer extends ResourceContainer<StorageTypePower> implements IEnergyComponent {
    ...
    private void configureRegulator() {
        this.setRegulator(new ThroughputRegulator.Limited<>(chemistry.maxChargeJoulesPerTick(this.getCapacity()),
    ...
    /**
     * Power storage components can only provide energy to the device if this is an
     * isolated or disconnected buffer, or if the caller is running on the power
     * service thread. Also must have contain energy!
     * <p>
     * 
     * {@inheritDoc}
     */
    ...
    /**
     * Power storage components can only accept energy from the device if this is an
     * output or isolated buffer, or if the caller is running on the power service
     * thread.
     * <p>
     * 
     * {@inheritDoc}
