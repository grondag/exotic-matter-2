 ...
 * when IStorage is being accessed this way, limit changes to server thread,
 * etc.
 */
public class ItemContainer extends ResourceContainer<StorageTypeStack> implements IItemHandler {
    ...
    /**
     * <i>If we have available capacity, then effectively one more slot available to
     * add another items not already here.</i><br>
     * <br>
     * 
     * {@inheritDoc}
     */
        ...
        try {
            return LogisticsService.ITEM_SERVICE.executor.submit(() -> {
                ...
                if (rwq != null) {
                    ...
                    if (result != null)
                        result.setCount((int) Math.min(Integer.MAX_VALUE, rwq.getQuantity()));
                }
                return result;
            }, true).get();
        } catch (Exception e) {
    ...
    /**
     * <i>Our storage doesn't care about stacking but need to honor slot that is
     * sent otherwise tend to get strangeness. </i><br>
     * <br>
     * 
     * {@inheritDoc}
     */
    ...
    public ItemStack insertItem(int slot, @Nonnull @Nullable ItemStack stack, boolean simulate) {
        ...
        try {
            return LogisticsService.ITEM_SERVICE.executor.submit(() -> {
                ...
                if (slot < slots.size()) {
                    // reject if trying to put in a mismatched slot - will force it to add to end
                    // slot
                    AbstractResourceWithQuantity<StorageTypeStack> rwq = slots.get(slot);
                    if (!((ItemResource) rwq.resource()).isStackEqual(stack))
                        return stack;
                }
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        // TODO: should not need to run this on service thread for private output
        // buffers
    ...
    public int getSlotLimit(int slot) {
        try {
            return LogisticsService.ITEM_SERVICE.executor.submit(() -> {
                ...
                if (slot < 0 || slot >= slots.size())
                    return (int) Math.min(Integer.MAX_VALUE, this.availableCapacity());
