...
public class ItemStorageListener implements IStorageAccess<StorageTypeStack> {
    ...
    /**
     * Domain if we are in domain mode, and last known domain of storage in other
     * modes.
     */
    ...
    /**
     * Used in connected mode to track which stores are currently connected to the
     * store in {@link #storage}. Includes the primary store. Null in domain mode.
     */
    ...
    private final static IFunction<ItemResourceDelegate, Integer> RESOURCE_HANDLE_MAPPER = new IFunction<ItemResourceDelegate, Integer>() {
        @Override
     ...
     */
    protected Key2List<ItemResourceDelegate, ItemResource, Integer> slots = new Key2List.Builder<ItemResourceDelegate, ItemResource, Integer>()
            .withPrimaryKey1Map(ItemResourceDelegate::resource).withUniqueKey2Map(RESOURCE_HANDLE_MAPPER).build();

    /**
     * Constructor for domain-level listener.
     * <p>
     * 
     * Creates a new storage listener, but does not subscribe. Subscription should
     * be done via the LogisticsService to ensure that the initial snapshot is
     * consistent with all later updates.
     * <p>
     */
     ...
     * @param connectedMode If true, will listen to given storage plus all
     *                      physically connected storage machines.
     */
    ...
    public ItemStorageListener(ItemContainer storage, EntityPlayerMP player) {
        ...
        this.stores = storage.isOn() ? new HashSet<IResourceContainer<StorageTypeStack>>() : ImmutableSet.of(storage);
     ...
     * inventory snapshot, register self for events, and send initial refresh to
     * client.
     */
    public void initialize() {
        ...
        PacketHandler.CHANNEL.sendTo(new PacketOpenContainerItemStorageRefresh(ImmutableList.copyOf(map.values()), this.totalCapacity, true), player);
    }
    ...
    /**
     * Adds store and adds or updates item delegates in the map. Have to use a map
     * because could have same bulkResource and thus same delegate for more than one
     * store. We only send a single delegate per bulkResource.
     */
    ...
    /**
     * Returns the delegate that was added or changed so that it may be sent to
     * client if desired. Does not update client.
     * <p>
     * 
     * Does not remove resources that are set to zero quantity because handle values
     * may still be retained on client.
     */
    ...
    public void onItemUpdate(ItemStoredUpdate event) {
        ...
        case STORE:
            if (this.stores.contains(event.storage)) {
                PacketHandler.CHANNEL.sendTo(new PacketOpenContainerItemStorageRefresh(ImmutableList.of(this.changeQuantity(event.resource, event.delta)),
                        this.totalCapacity, false), player);
            }
            break;

        case DOMAIN:
            PacketHandler.CHANNEL.sendTo(
                    new PacketOpenContainerItemStorageRefresh(ImmutableList.of(this.changeQuantity(event.resource, event.delta)), this.totalCapacity, false),
                    player);
            break;
    ...
    public void afterItemStorageConnect(AfterItemStorageConnect event) {
        ...
        assert LogisticsService.ITEM_SERVICE.confirmServiceThread() : "Storage listener events outside service thread.";
        ...
        case CONNECTED: {
            ...
            if (event.storage.isOn() && !this.stores.contains(event.storage)
                ...
                PacketHandler.CHANNEL.sendTo(new PacketOpenContainerItemStorageRefresh(ImmutableList.copyOf(map.values()), this.totalCapacity, false), player);
            }
            break;
        }
    ...
    public void beforeItemStorageDisconnect(BeforeItemStorageDisconnect event) {
        ...
        case CONNECTED: {
            if (event.storage == this.storage) {
                ...
                PacketHandler.CHANNEL.sendTo(new PacketOpenContainerItemStorageRefresh(builder.build(), this.totalCapacity, false), player);
    ...
    /**
     * Will deregister for events if haven't already and release bulkResource
     * references.
     */
    public void die() {
        if (!this.isDead) {
    ...
    /**
     * For non-domain listeners, limits to connectivity from the primary storage
     * device. Always empty for domain listener.
     */
    public ImmutableList<IResourceContainer<StorageTypeStack>> findSpaceFor(IResource<StorageTypeStack> resource) {
        return this.storage == null ? ImmutableList.of() : IStorageAccess.super.findSpaceFor(resource, this.storage.device());
    }

    /**
     * For non-domain listeners, limits to connectivity from the primary storage
     * device. Always fails (returns 0) for domain listener.
     */
    public long add(IResource<StorageTypeStack> resource, final long howMany, boolean simulate, @Nullable NewProcurementTask<StorageTypeStack> request) {
        return this.storage == null ? 0 : IStorageAccess.super.add(resource, howMany, simulate, request, this.storage.device());
    }

    @Override
    public ImmutableList<IResourceContainer<StorageTypeStack>> getLocations(IResource<StorageTypeStack> resource) {
        return this.mode == Mode.DOMAIN ? this.domain.getCapability(ItemStorageManager.class).getLocations(resource)
