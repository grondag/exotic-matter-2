...
public class ResourceContainer<T extends StorageType<T>> implements IResourceContainer<T> {
    ...
    /**
     * If usage is {@link ContainerUsage#PUBLIC_BUFFER_OUT} will be a private output
     * buffer that is co-serialized. Otherwise always null.
     * 
     * For output buffers, calls to
     * {@link #addLocally(IResource, long, boolean, boolean, NewProcurementTask)}
     * can happen outside the service thread, and contents will be automatically
     * moved to the main buffer on that thread.
     * <p>
     * 
     * Size of the internal buffer is set equal to capacity of the public output
     * buffer when contents are moved.
     */
    ...
    /**
     * Make this something other than the dummy regulator during constructor if you
     * want limits or accounting.
     */
    ...
    public ResourceContainer(T storageType, IDevice owner, ContainerUsage usage, int maxSlots) {
        ...
        this.slots = maxSlots == 1 ? new ResourceSlotsSingle<T>() : new ResourceSlotsMulti<T>();

        this.localSlots = usage == ContainerUsage.PUBLIC_BUFFER_OUT ? new ResourceContainer<T>(storageType, owner, ContainerUsage.PRIVATE_BUFFER_OUT, maxSlots)
                : null;
    }
    ...
    public void setContentPredicate(Predicate<IResource<T>> predicate) {
        if (this.predicate != predicate && predicate != null) {
            if (this.used == 0) {
                ...
                assert false : "Attempt to configure non-empty bulkResource container.";
        }
    }
    ...
    /**
     * For output buffers, ensures private buffer has available capacity equal to
     * available capacity of public container.
     */
    ...
    private long addImpl(IResource<T> resource, long howMany, boolean simulate, boolean allowPartial, @Nullable NewProcurementTask<T> request) {
        if (howMany < 1 || !this.isResourceAllowed(resource))
            return 0;

        // if we are already at max slot count, prevent adding another slot
        if (this.slots.size() >= this.maxSlotCount && this.slots.getQuantity(resource) == 0)
            return 0;
        ...
        if (added < 1 || (!allowPartial && added != howMany))
            return 0;
    ...
    private long takeUpToImpl(IResource<T> resource, long limit, boolean simulate, boolean allowPartial, NewProcurementTask<T> request) {
        if (limit < 1)
            return 0;
    ...
    public List<AbstractResourceWithQuantity<T>> slots() {
        return this.slots.isEmpty() ? ImmutableList.of() : ImmutableList.copyOf(this.slots);
    }
    ...
    /**
     * Can be called from device outside service thread to store content. Contents
     * will be automatically be moved to public buffer on service queue.
     * <p>
     * 
     * Contents in private buffer are serialized and if game crashes before they
     * move to public buffer, will be added to public buffer at time of
     * deserialization, if they fit within capacity.
     */
    ...
    /**
     * Tries to move contents of private local buffer to public buffer. Called on
     * service thread after every attempt to add to the local buffer, and called
