...
public class StorageResourceManager<T extends StorageType<T>> implements INewTaskListener {
    ...
    /**
     * True if listener notification is needed. Set false at start of notification
     * process. Prevents redundant spamming of listeners when lots of changes
     * happening at once.
     */
     ...
     * the storage, if non-null. Useful when fabrication is storing items made for
    ...
    public synchronized long setAllocation(NewProcurementTask<T> request, long requestedAllocation) {
        ...
        if (this.allocations == null) {
            ...
            if (pair.getLeft() == request) {
                ...
                assert requestedAllocation > 0 : "Logic error resulted in tracking a zero allocation";
        ...
        if (this.quantityAvailable() != startingAvailable)
            this.notifyListenersOfAvailability();
    ...
    /**
     * Called by storage when resources are removed. If request is non-null, then
     * the amount taken reduces any allocation to that request.
     */
    public synchronized void notifyTaken(IResourceContainer<T> storage, long taken, @Nullable NewProcurementTask<T> request) {
        if (taken == 0)
            return;

        assert storage.containerUsage().isListed : "Notification for unlisted storage";
        ...
        if (request != null)
            this.changeAllocation(request, -taken);
        ...
        if (this.quantityStored < this.quantityAllocated) {
            if (this.allocations == null) {
                ...
                List<Entry<NewProcurementTask<T>>> list = map.object2LongEntrySet().stream().sorted(new Comparator<Entry<NewProcurementTask<T>>>() {
                    @Override
        ...
        // asynch - returns immediately
        if (startingAvailable != this.quantityAvailable())
            this.notifyListenersOfAvailability();
    }
    ...
    public synchronized void notifyAdded(IResourceContainer<T> storage, long added, @Nullable NewProcurementTask<T> request) {
        ...
        if (this.stores.addIfNotPresent(storage)) {
            ...
            assert storage.getQuantityStored(resource) == added : "Storage Resource Manager encountered request to add a quanity in a new storage"
        ...
        // update allocation if request provided
        if (request != null)
            this.changeAllocation(request, added);
    ...
    /**
     * Returns all locations where the resource is stored, irrespective of
     * allocation.
     */
    ...
    public synchronized void registerResourceListener(IStorageResourceListener<T> listener) {
        if (this.listeners == null) {
            ...
            if (this.listeners != listener) {
                // upgrade to list
                SimpleUnorderedArrayList<IStorageResourceListener<T>> list = new SimpleUnorderedArrayList<IStorageResourceListener<T>>();
                list.add((IStorageResourceListener<T>) this.listeners);
                list.add(listener);
            }
        } else {
            // should already be using list
            ((SimpleUnorderedArrayList<IStorageResourceListener<T>>) this.listeners).addIfNotPresent(listener);
        }
    }
    ...
    public synchronized void unregisterResourceListener(IStorageResourceListener<T> listener) {
        if (this.listeners == null) {
            ...
            @SuppressWarnings("unchecked")
            SimpleUnorderedArrayList<IStorageResourceListener<T>> list = (SimpleUnorderedArrayList<IStorageResourceListener<T>>) this.listeners;
    ...
    /**
     * Called whenever availability changes. Returns immediately and schedules
     * notification to happen asynchronously. This prevents a chain of
     * notification-driven changes involving listener actions that would all have to
     * be resolved before the initiating listener gets a result.
     * <p>
     * 
     * Sets the {@link #isListenerNotificationDirty} flag so that it is not
     * necessary to do this in caller.
     */
    @SuppressWarnings({ "unchecked" })
    protected void notifyListenersOfAvailability() {
        ...
        Simulator.CONTROL_THREAD.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (!StorageResourceManager.this.isListenerNotificationDirty)
                    return;
    ...
    /**
     * If true, has no storages and no listeners and can thus be safely discarded by
     * the storage manager. Implies all quantities are zero.
     */
    @SuppressWarnings("unchecked")
    public boolean isEmpty() {
        if (this.listeners == null && this.stores.isEmpty()) {
            assert (this.allocations == null
                    || ((Object2LongOpenHashMap<NewProcurementTask<T>>) this.allocations).isEmpty()) : "StorageResourceManager empty with non-null allocations";

            assert (this.quantityAllocated() == 0) : "StorageResourceManager empty with non-zero allocations";

            assert (this.quantityAvailable() == 0) : "StorageResourceManager empty with non-zero available";

            assert (this.quantityStored() == 0) : "StorageResourceManager empty with non-zero stored";
