...
public class FluidContainer extends ResourceContainer<StorageTypeFluid> implements IFluidHandler {
    /**
     * Content key should be either at BulkBufferPurpose entry or a specific fluid
     * resource. If it is specific fluid resource, this container will only accept
     * that fluid.
     */
    ...
    private class TankProps implements IFluidTankProperties {
        ...
        public @Nullable FluidStack getContents() {
            if (FluidContainer.this.usedCapacity() == 0)
                return null;

            FluidStack result = (this.resource).sampleFluidStack().copy();
            result.amount = (int) Math.min(VolumeUnits.nL2Liters(FluidContainer.this.getQuantityStored(this.resource)), Integer.MAX_VALUE);
            return result;
        }

        @Override
        public int getCapacity() {
            return (int) Math.min(VolumeUnits.nL2Liters(FluidContainer.this.getCapacity()), Integer.MAX_VALUE);
        }

        @Override
        public boolean canFill() {
            return this.resource == null ? FluidContainer.this.availableCapacity() >= VolumeUnits.LITER.nL
                    : FluidContainer.this.availableCapacityFor(this.resource) >= VolumeUnits.LITER.nL;
        }

        @Override
        public boolean canDrain() {
            return this.resource == null ? false : FluidContainer.this.getQuantityStored(this.resource) >= VolumeUnits.LITER.nL;
        }
        ...
        public boolean canDrainFluidType(@Nullable FluidStack fluidStack) {
            ...
            return FluidContainer.this.takeUpTo(FluidResource.fromStack(fluidStack), VolumeUnits.LITER.nL, true) == VolumeUnits.LITER.nL;
        }
    };
    ...
    public int fill(@Nullable FluidStack stack, boolean doFill) {
        ...
        if (!this.isResourceAllowed(resourceIn))
            return 0;
