...
public interface IResourceContainer<T extends StorageType<T>> extends ISizedContainer, ITypedStorage<T>, IDeviceComponent, IReadWriteNBT {

    /**
     * Returned bulkResource stacks are disconnected from this collection. Changing
     * them will have no effect on storage contents.
     */
    ...
    /**
     * Implemented for some container types to collect stats and/or limit rate of
     * input/output.
     */
    ThroughputRegulator<T> getRegulator();

    /**
     * Used by some container types to collect stats and/or limit rate of
     * input/output.
     */
    ...
    /**
     * Increases quantityStored and returns quantityStored actually added. If
     * simulate==true, will return forecasted result without making changes. Thread
     * safety depends on implementation.
     * <p>
     * 
     * If request is non-null and not simulated, and this container supports
     * allocation, the amount added will be allocated (domain-wide) to the given
     * request.
     */
    public long add(IResource<T> resource, long howMany, boolean simulate, boolean allowPartial, @Nullable NewProcurementTask<T> request);

    /**
     * Alternate syntax for
     * {@link #add(IResource, long, boolean, boolean, NewProcurementTask)} that
     * assumes allowPartial == true.
     */
    public default long add(IResource<T> resource, long howMany, boolean simulate, @Nullable NewProcurementTask<T> request) {
     ...
     * Alternate syntax for
     * {@link #add(IResource, long, boolean, boolean, NewProcurementTask)} that
     * assumes allowPartial == true and request == null.
     */
    public default long add(IResource<T> resource, long howMany, boolean simulate) {
        return this.add(resource, howMany, simulate, true, null);
    }

    /**
     * Alternate syntax for
     * {@link #add(IResource, long, boolean, NewProcurementTask)}
     */
     ...
     * 
     * If request is non-null and not simulated, and this container supports
     * allocation, the amount taken will reduce any allocation (domain-wide) to the
     * given request.
     */
    public long takeUpTo(IResource<T> resource, long limit, boolean simulate, boolean allowPartial, @Nullable NewProcurementTask<T> request);

    /**
     * Alternate syntax for
     * {@link #takeUpTo(IResource, long, boolean, boolean, NewProcurementTask)}
     * which assumed allowPartial == true.
     */
    public default long takeUpTo(IResource<T> resource, long limit, boolean simulate, @Nullable NewProcurementTask<T> request) {
     ...
     */
    public default long takeUpTo(IResource<T> resource, long limit, boolean simulate) {
    ...
    /**
     * Submits privileged execution to service thread and waits. For use by in-game
     * user interactions. Fulfills constraint that all storage interactions must
     * occur on service thread for that storage type.
     */
    public default long addInteractively(IResource<T> resource, long howMany, boolean simulate) {
        try {
    ...
    /**
     * Setting to a non-null value will configure this container to accept only
     * resources that match the given predicate. Setting to null will remove this
     * constraint. Container must be empty when set to a non-null value.
     */
