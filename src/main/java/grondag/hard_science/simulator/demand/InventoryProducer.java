...
public class InventoryProducer<V extends StorageType<V>> implements IProducer<V>, IStorageResourceListener<V> {
    /**
     * Broker that is tracking demand for our resource. If null, means
     * {@link #tearDown()} has run and we should ignore any incoming requests.
     */
    ...
    /**
     * True while we are filling demand - means we should ignore callbacks to
     * {@link #onAvailabilityChange(IResource, long)} from storage system because we
     * are causing them. Would get into infinite loop otherwise.
     */
    ...
    public void notifyNewDemand(IBroker<V> broker, NewProcurementTask<V> request) {
        assert this.broker != null : "Inventory Producer received new demand after tear down.";
    ...
    /*
     * Synchronization is to prevent being called on a different thread from {@link
     * #fillDemand(IProcurementRequest)} so that we can reliably ignore callback
     * from storage while we are allocating demand.
     */
    ...
    /**
     * Check for unmet demand we can fill if new inventory is added.
     * <p>
     * 
     * Synchronization is to prevent being called on a different thread from
     * {@link #fillDemand(IProcurementRequest)} so that we can reliably ignore
     * callback from storage while we are allocating demand.
     * 
     * {@inheritDoc}
     */
    @Override
