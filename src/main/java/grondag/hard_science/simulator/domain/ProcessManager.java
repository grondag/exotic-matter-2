...
public class ProcessManager implements IDomainCapability {
    ...
    public abstract class ProcessInfo {
        ...
        public long reserveStockLevel() {
            return this.reserveStockLevel;
        }

        public long targetStockLevel() {
        ...
        protected void normalize() {
            if (this.reserveStockLevel < 0)
                this.reserveStockLevel = 0;
        ...
        public long stockIntervalSize() {
            return this.targetStockLevel - this.reserveStockLevel;
        }
         ...
         * 0 to 1.0 On hand it between reserve stocking level and target stocking level
         * <p>
        ...
        public float availabilityFactor() {
            ...
            if (onHand > this.targetStockLevel)
                return onHand - this.targetStockLevel + 1;
    ...
    public class FluidProcessInfo extends ProcessInfo {
        ...
        public IResource<StorageTypeFluid> resource() {
    ...
    public class IngredientProcessInfo extends ProcessInfo {
        ...
        @Override
        public long onHand() {
            List<AbstractResourceWithQuantity<StorageTypeStack>> stocked = domain.getCapability(ItemStorageManager.class)
                    .findEstimatedAvailable(this.ingredient);
    ...
    public static Ingredient readIngredient(String ingString) {
        ...
        if (itemArgs[0].equals("ore")) {
            ...
            if (itemArgs.length == 3) {
                // metadata provided
                return Ingredient.fromStacks(new ItemStack(item, 1, Integer.parseInt(itemArgs[2])));
            } else {
                // no metadata
    ...
    /**
     * Searches for ingredients that match the given item resource and chooses the
     * one with the highest target levels.
     */
    public IngredientProcessInfo getInfo(ItemResource resource) {
        ...
        for (IngredientProcessInfo pi : this.ingredientInfos.values()) {
            if (pi.ingredient.test(stack)) {
                if (result == null || pi.targetStockLevel < result.targetStockLevel || pi.reserveStockLevel < result.reserveStockLevel) {
                    result = pi;
                }
            }
        }
            ...
            if (tags != null && !tags.hasNoTags()) {
                for (int i = 0; i < tags.tagCount(); ++i) {
                    try {
                        ...
                        if (pi.resource != null)
                            this.fluidInfos.put(pi.resource, pi);
                    } catch (Exception e) {
