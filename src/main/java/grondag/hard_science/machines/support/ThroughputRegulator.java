...
public abstract class ThroughputRegulator<T extends StorageType<T>> {
    /**
     * Has no internal state, enforces no limits and does no accounting. Use when
     * you don't want to regulate and don't want to check for null.
     */
    @SuppressWarnings("rawtypes")
    public static final ThroughputRegulator DUMMY = new Dummy();

    /**
     * If this regulator has input constraints, limits the requested amount to those
     * constraints and updates internal accounting if applicable.
     * <p>
     * 
     * @param requested    desired input amount
     * @param isSimulated  if false, will not update internal accounting
     * @param allowPartial if false will return zero if constraints don't permit
     *                     full amount
    ...
    /**
     * If this regulator has output constraints, limits the requested amount to
     * those constraints and updates internal accounting if applicable.
     * <p>
     * 
    ...
    public static class Limited<T extends StorageType<T>> extends ThroughputRegulator.Tracking<T> {
        ...
        public synchronized long limitInput(final long requested, boolean isSimulated, boolean allowPartial) {
            ...
            if (result != requested && !allowPartial)
                return 0;
