...
/**
 * Answers the question: if I have machine that can process two things, and I
 * have both things on hand, which thing should I process first?
 * <p>
 * 
 * General rules Process resources that satisfy higher-priority requests first
 * Process resources that will satisfy the largest number of requests. Don't
 * process a resource if an immediate output has reached max backlog or storage
 * is full.
 *
 *
 */
public class MicronizerInputSelector implements IDomainMember {
     ...
     * 
     * 2) If inputs produce same output or outputs with same demand level, use
     * inputs above target stock level first. Pick input with the highest absolute
     * overstock amount.
     * 
     * 3) If all inputs for an output are below target stocking levels, pick the
     * input most available relative to the target.
     * 
     */
    private void updateBacklogIfStale() {
        if (Simulator.currentTick() < nextUpdateTick)
            return;
        ...
        // find all inputs
        List<AbstractResourceWithQuantity<StorageTypeStack>> candidates = this.owner.getDomain().getCapability(ItemStorageManager.class)
                .findEstimatedAvailable(MicronizerRecipe.INPUT_RESOURCE_PREDICATE);
        ...
        this.bestInputs = allInputs.stream().sorted(new Comparator<BasicEntry<IResource<StorageTypeStack>>>() {
            @Override
        ...
        }).map(p -> p.getKey()).collect(ImmutableList.toImmutableList());
    }

    /**
     * Finds best inputs for micronizing, and an upper bound for how much of it
     * should be processed. Upper bound because could exist multiple items that
     * satisfy the need.
     * 
     * TODO: Use request priority as an input - right now only considers domain
     * stocking levels and priorities
     */
    ...
    /**
     * 
     * Estimated number of crusher cycles that need to run to reach target output
     * levels.
     */
    ...
    public Future<NewProcurementTask<StorageTypeStack>> requestInput(MicronizerMachine machine) {
        return LogisticsService.ITEM_SERVICE.executor.submit(new Callable<NewProcurementTask<StorageTypeStack>>() {
            ...
            public NewProcurementTask<StorageTypeStack> call() throws Exception {
                ...
                if (!machine.itemTransport().hasAnyCircuit())
                    return null;

                // look for output resources that are in quantity
                // for those in quantity, take the highest priority
                List<IResource<StorageTypeStack>> candidates = findBestInputs();
                ...
                if (!candidates.isEmpty()) {
                    ...
                    for (IResource<StorageTypeStack> res : candidates) {
                        ...
                        if (machine.getBufferManager().itemInput().availableCapacityFor(res) == 0)
                            continue;

                        IResourceContainer<StorageTypeStack> store = sources.get(0);

                        result = new SimpleProcurementTask<>(machine.getDomain().getCapability(TransientTaskContainer.class), res, 1);
