...
public class PolyethyleneFuelCell extends AbstractGenerator {
    ...
    private void setup(CubeSize plateSize, int plateQuantity, boolean hasThermalCapture) {
        ...
        long maxFuelConsumptionNanoLitersPerSimulatedSecond = plateSize.faceSurfaceArea_micrometer2 * this.plateQuantity
                * MachinePower.POLYETHYLENE_MAX_CONVERSION_RATE_MICROMETERS / VolumeUnits.CUBIC_MICROMETERS_PER_NANOLITER;
        ...
        this.setMaxOutputJoulesPerTick(
                (long) (maxFuelConsumptionNanoLitersPerTick * MachinePower.JOULES_PER_POLYETHYLENE_NANOLITER * this.cellType.conversionEfficiency));
    }
    ...
    protected long generateImplementation(long maxOutput, boolean allowPartial, boolean simulate) {
        ...
        if (fuel == 0) {
            if (!simulate)
                fuelBuffer.blame(ModBulkResources.HDPE.fluidResource());
            return 0;
        }
        ...
        if (energy > 0) {
            ...
            if (!allowPartial && energy < maxOutput)
                return 0;

            // consume fuel if not simulating
            if (!simulate)
                fuelBuffer.takeUpTo(ModBulkResources.HDPE.fluidResource(), fuel, false);
        }
        return energy;
    }
    ...
    public void deserializeNBT(@Nullable NBTTagCompound tag) {
        setup(Useful.safeEnumFromOrdinal(tag.getInteger(NBT_MACHINE_FUEL_CELL_PLATE_SIZE), CubeSize.THREE),
                Math.max(1, tag.getInteger(NBT_MACHINE_FUEL_CELL_PLATE_COUNT)), tag.getBoolean(NBT_MACHINE_FUEL_CELL_HAS_THERMAL_CAPTURE));
    }
