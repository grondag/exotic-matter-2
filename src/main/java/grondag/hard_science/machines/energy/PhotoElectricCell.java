...
public class PhotoElectricCell extends AbstractGenerator {
    ...
    /**
     * The number of joules we should generate per tick at full brightness in our
     * location. Factors in biome / dimension factors and base efficiency of the
     * cell.
     */
    private float joulesPerTickBrightness = 0;

    /**
     * Tick when we will request an update of joulesPerTickBrightness to account for
     * changes in the environment. (Placing or breaking blocks above us, for
     * example.)
     */
    ...
    public PhotoElectricCell(IDevice owner) {
        ...
        this.setMaxOutputJoulesPerTick(
                (long) (MachinePower.DAILY_INSOLATION_MAX_JOULES * MachinePower.PHOTO_ELECTRIC_EFFICIENCY / MachinePower.TOTAL_DAILY_BRIGHTNESS_FACTOR));
    }
    ...
    private void requestRefreshFromWorld() {
        ...
        WorldTaskManager.enqueue(new IWorldTask() {
            ...
            public int runInServerTick(int maxOperations) {
                ...
                float factor = MachinePower.insolationFactor(device().getLocation()) * MachinePower.DAILY_INSOLATION_MAX_JOULES
                        * MachinePower.PHOTO_ELECTRIC_EFFICIENCY / MachinePower.TOTAL_DAILY_BRIGHTNESS_FACTOR;
    ...
    protected long generateImplementation(long maxOutput, boolean allowPartial, boolean simulate) {
        ...
        if (this.joulesPerTickBrightness == 0)
            return 0;
