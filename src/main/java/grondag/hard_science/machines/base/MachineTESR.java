...
public class MachineTESR extends SuperBlockTESR {
    ...
    public void render(@Nonnull SuperTileEntity te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        int displayAlpha = (int) (alpha
                * (MathHelper.clamp((1 - (Math.sqrt(mte.getLastDistanceSquared()) - Configurator.MACHINES.machineMaxRenderDistance) / 4) * 255, 0, 255)));
        ...
        // TE will send keepalive packets to server to get updated machine status for
        // rendering
        ...
        // the .5 is to move origin to block center so that we can rotate to correct
        // facing
        ...
        MachineControlRenderer.renderSpriteInBounds(tessellator, buffer, RenderBounds.BOUNDS_SYMBOL, mte.getSymbolSprite(), (displayAlpha << 24) | 0xFFFFFF,
                Rotation.ROTATE_NONE);
        MachineControlRenderer.renderMachineText(tessellator, buffer, FontHolder.FONT_RENDERER_LARGE, RenderBounds.BOUNDS_NAME, mte.clientState().machineName,
                HorizontalAlignment.CENTER, white);

        if (mte.clientState().hasOnOff) {
            MachineControlRenderer.renderBinarySprite(tessellator, buffer, RenderBounds.BOUNDS_ON_OFF, ModModels.TEX_MACHINE_ON_OFF, mte.clientState().isOn(),
                    white);
        }
    ...
    /**
     * Override if device needs to render anything special. Default is NOOP.
     */
