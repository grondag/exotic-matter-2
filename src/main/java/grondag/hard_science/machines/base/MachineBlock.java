...
public abstract class MachineBlock extends SuperBlockPlus implements IMachineBlock {
    ...
    /**
     * Default machine implementation only handles single-block machines.
     * <p>
     * 
     * {@inheritDoc}
     */
    ...
    public void onBlockClicked(@Nonnull World worldIn, @Nonnull BlockPos pos, @Nonnull EntityPlayer playerIn) {
        ...
        if (machineTE.machine() == null)
            return;

        if (!machineTE.machine().hasOnOff() && !machineTE.machine().hasRedstoneControl())
            return;
        ...
        if (machineTE.getCachedModelState().getAxisRotation().horizontalFace != side)
            return;
        ...
        if (machineTE.machine().hasOnOff() && RenderBounds.BOUNDS_ON_OFF.contains(faceX, faceY)) {
            if (machineTE.togglePower((EntityPlayerMP) playerIn)) {
                worldIn.playSound(null, pos.getX() + .5f, pos.getY() + .5f, pos.getZ() + .5f, SoundEvents.UI_BUTTON_CLICK, SoundCategory.PLAYERS, .2f,
                        ((worldIn.rand.nextFloat() - worldIn.rand.nextFloat()) * .7f + 1) * 2);
            }
    ...
    /**
     * On server-side after machine block has been placed will restore machine state
     * from stack (if present) or create new machine state and register with
     * simulation.
     * <p>
     * 
     * Relies on machine state to be saved by {@link #writeModNBT(NBTTagCompound)}
     * for harvested machines.
     * <p>
     * 
     * New machines are added to the placer's domain. Machines with an existing
