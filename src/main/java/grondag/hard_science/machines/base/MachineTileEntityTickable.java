...
public class MachineTileEntityTickable extends MachineTileEntity implements ITickable {
     ...
     * Difference from {@link #playerUpdateTicks} is that {@link #playerUpdateTicks}
     * controls <i>when</i> update occurs. {@link #isPlayerUpdateNeeded} contols
     * <i>if</i> update occurs.
     */
    ...
    private class PlayerListener extends KeyedTuple<EntityPlayerMP> {
        ...
        /**
         * Returns true if this listener should be removed because it has timed out or
         * player has disconnected.
         */
        private boolean checkForRemoval(long currentTime) {
            return this.key.hasDisconnected() || (currentTime > this.goodUntilMillis && !isOpenContainerForPlayer(this.key));
        }
    }
...
//    }
    ...
    public void markPlayerUpdateDirty(boolean isUrgent) {
        if (this.listeningPlayers == null || this.listeningPlayers.isEmpty())
            return;
    ...
    public void update() {
        if (world.isRemote) {
            ...
            if (time >= this.nextPlayerUpdateMilliseconds) {
                PacketMachineStatusUpdateListener packet = this.machine() == null ? null : this.createMachineStatusUpdate();
                ...
                while (i < listeningPlayers.size()) {
                    ...
                    if (listener.checkForRemoval(time)) {
                        ...
                        if (packet != null)
                            PacketHandler.CHANNEL.sendTo(packet, listener.key);
                        i++;
                    }
                }
