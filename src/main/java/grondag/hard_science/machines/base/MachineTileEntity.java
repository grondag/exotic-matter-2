...
public class MachineTileEntity extends XmBlockEntity {
     ...
     * {@link #getDistanceSq(double, double, double)}
     */
    private double lastDistanceSquared;

    /**
     * /!\ Lazy instantiation - do not use directly. Use {@link #clientState()}
     * instead.
     */
    protected MachineClientState clientState = null;

    /**
     * On server, next time update should be sent to players. On client, next time
     * keepalive request should be sent to server.
     */
    ...
    /**
     * Caches machine instance, and notifies it of loaded tile entity. Will be null
     * during early stages of block placement.
     */
    @Nullable
    ...
    protected boolean isOpenContainerForPlayer(EntityPlayerMP player) {
        ...
        return openContainer != null && openContainer instanceof MachineContainer && ((MachineContainer) openContainer).tileEntity() == this;
    }

    /**
     * Used by GUI and TESR to draw machine's symbol. Actual implementation is in
     * block, but this provides convenient access.
     */
    @Environment(EnvType.CLIENT)
    ...
    /**
     * Called server-side after machine block has been placed to avoid need for a
     * machine look up and to immediately notify machine of loaded tile entity.
     */
    ...
    /**
     * Returns cached value from {@link #getDistanceSq(double, double, double)} that
     * is called each render pass by TE render dispatcher so that we don't have to
     * compute again when needed in TESR.
     */
    @Environment(EnvType.CLIENT)
    ...
    public boolean isRemote() {
        return this.world == null ? FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT : this.world.isRemote;
    }

    /**
     * Writes the machine state for use in
     * {@link #restoreMachineFromStack(ItemStack, Domain)}. Stores it in server-side
     * tag so that large states don't get sent to client.
     * <p>
     * 
     * Machine state stored here is <em>not</em> used in
     * {@link #readModNBT(NBTTagCompound)}.
     * <p>
     * 
     * {@inheritDoc}
     */
    ...
    public void writeModNBT(NBTTagCompound compound) {
        ...
        if (!this.world.isRemote) {
            if (this.machine() != null) {
                SuperTileEntity.getServerTag(compound).setTag(NBT_DEVICE_STATE, this.machine().serializeNBT());
            }
        }
    }

    /**
     * Called to notify observing players of an update to machine status. If
     * isUrgent == true will send update on next tick. isUrgent should be used for
     * big visible changes, like power on/off<br>
     * <br>
     * 
     * If {@link #playerUpdateMilliseconds} has been set to a negative value, will
     * have no effect unless isUrgent == true. <br>
     * <br>
     * 
     * Seldom called directly except with required updates because is called
     * automatically by {@link #markDirty()}
     * <p>
     * 
    ...
    /**
     * If isRequired == true, will send client updates more frequently. Does not
     * downgrade player to non-focused if previously added as focused. Will cause an
     * immediate player refresh if the listener is new or if existing listener goes
     * from non-urgent to urgent.
     * <p>
     * 
     * If this TE is not tickable, will simply send an immediate update of current
     * state. Non-tickable machine TEs are assumed to have infrequent display
     * changes.
     */
    ...
    public boolean togglePower(EntityPlayerMP player) {
        if (!this.hasWorld() || this.isInvalid() || this.machine() == null || !this.machine().hasOnOff())
            return false;
    ...
    /**
     * Handles packet from player to toggle redstone control on or off. Returns
     * false if denied.
     */
    public boolean toggleRedstoneControl(EntityPlayerMP player) {
        ...
        if (this.world.isRemote) {
            ...
            // called by packet handler on server side
            if (this.machine() == null && !this.machine().hasRedstoneControl())
                return false;
    ...
    /**
     * Called when player is looking directly at this machine, within machine
     * rendering distance.
     */
    @Environment(EnvType.CLIENT)
    ...
    public @Nullable <T> T getCapability(@Nonnull Capability<T> capability, @Nullable EnumFacing facing) {
        if (capability == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY && this.machine().hasItemStorage()) {
            return CapabilityItemHandler.ITEM_HANDLER_CAPABILITY.cast((IItemHandler) this.machine().itemStorage());
        } else if (capability == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY && this.machine().hasFluidStorage()) {
