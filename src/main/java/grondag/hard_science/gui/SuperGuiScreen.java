...
//        }
    ...
    private void updateColors(PaintLayer layer) {
        ...
        textureTabBar[layer.ordinal()].borderColor = BlockColorMapProvider.INSTANCE.getColorMap(colorPicker[layer.ordinal()].getColorMapID())
        ...
        if (layer == PaintLayer.BASE) {
            ...
            int baseColor = modelState.isEmissive(PaintLayer.BASE) ? map.getColor(EnumColorMap.LAMP) : map.getColor(EnumColorMap.BASE);
    ...
    public void initGui() {
        ...
        buttonWidth = Math.max(fontRenderer.getStringWidth(STR_ACCEPT), fontRenderer.getStringWidth(STR_CANCEL)) + CONTROL_INTERNAL_MARGIN
                + CONTROL_INTERNAL_MARGIN;
        ...
        if (textureTabBar == null) {
            ...
            for (int i = 0; i < PaintLayer.SIZE; i++) {
                TexturePicker t = (TexturePicker) new TexturePicker(new ArrayList<ITexturePalette>(), xStart + CONTROL_EXTERNAL_MARGIN, yStart + 100)
                        .setVerticalWeight(5);
        ...
        if (mainPanel == null) {
            ...
            rightPanel = (VisibilityPanel) new VisibilityPanel(true).setOuterMarginWidth(CONTROL_EXTERNAL_MARGIN).setInnerMarginWidth(CONTROL_EXTERNAL_MARGIN)
                    .setHorizontalWeight(5).setBackgroundColor(GuiControl.CONTROL_BACKGROUND);

            group_base = rightPanel.createVisiblityGroup(PaintLayer.BASE.localizedName());
            Panel tempV = new Panel(true).addAll(fullBrightToggle[PaintLayer.BASE.ordinal()], baseTranslucentToggle).setHorizontalWeight(2);
            ...
            tempV = new Panel(true).addAll(middleToggle, fullBrightToggle[PaintLayer.MIDDLE.ordinal()]).setHorizontalWeight(2);
            ...
            tempV = new Panel(true).addAll(outerToggle, fullBrightToggle[PaintLayer.OUTER.ordinal()]).setHorizontalWeight(2);
            ...
            tempV = new Panel(true).addAll(fullBrightToggle[PaintLayer.LAMP.ordinal()], lampTranslucentToggle).setHorizontalWeight(2);
            ...
            Panel leftPanel = (Panel) new Panel(true).setInnerMarginWidth(CONTROL_EXTERNAL_MARGIN)
                    .add(new Panel(true).setOuterMarginWidth(CONTROL_EXTERNAL_MARGIN).add(itemPreview).setBackgroundColor(GuiControl.CONTROL_BACKGROUND)
                            .setVerticalWeight(1))
                    .add(new Panel(true).add(selector).setBackgroundColor(GuiControl.CONTROL_BACKGROUND).setVerticalWeight(3)).setWidth(100)
                    .setHorizontalLayout(Layout.FIXED).resize(0, 0, (xSize - CONTROL_EXTERNAL_MARGIN) * 2.0 / 7.0, 1);

            mainPanel = (Panel) new Panel(false).setOuterMarginWidth(0).setInnerMarginWidth(CONTROL_EXTERNAL_MARGIN).resize(xStart + CONTROL_EXTERNAL_MARGIN,
                    yStart + CONTROL_EXTERNAL_MARGIN, xSize - CONTROL_EXTERNAL_MARGIN * 2, ySize - CONTROL_EXTERNAL_MARGIN * 3 - buttonHeight);
            ...
            mainPanel.resize(xStart + CONTROL_EXTERNAL_MARGIN, yStart + CONTROL_EXTERNAL_MARGIN, xSize - CONTROL_EXTERNAL_MARGIN * 2,
                    ySize - CONTROL_EXTERNAL_MARGIN * 3 - buttonHeight);
        }
    }
...
//        translucencyPicker.setTranslucency(modelState.getTranslucency());
        ...
        for (PaintLayer layer : PaintLayer.VALUES) {
            ...
            t.showSelected();
            t.borderColor = modelState.isEmissive(layer) ? ColorHelper.lampColor(modelState.getColorARGB(layer)) : modelState.getColorARGB(layer);
            t.baseColor = modelState.isEmissive(PaintLayer.BASE) ? ColorHelper.lampColor(modelState.getColorARGB(PaintLayer.BASE))
                    : modelState.getColorARGB(PaintLayer.BASE);

            ColorPicker c = colorPicker[layer.ordinal()];

            // FIXME: not going to work - commented to allow compile after removing color
            // map from model state
