...
public class ItemStackPicker extends TabBar<ItemResourceDelegate> {
    ...
    public ItemStackPicker(List<ItemResourceDelegate> items, FontRenderer fontRenderer, IClickHandler<ItemResourceDelegate> clickHandler) {
        super(items);
    ...
    @Override
    protected void drawItem(ItemResourceDelegate item, Minecraft mc, RenderItem itemRender, double left, double top, float partialTicks,
    ...
    protected void drawQuantity(long qty, int left, int top) {
        ...
        GuiUtil.drawAlignedStringNoShadow(this.fontRenderer, qtyLabel, 0, 0, 16 * 10 / 6, this.getCaptionHeight() * 10 / 6, 0xFFFFFFFF,
