    ...
    protected void drawContent(IGuiRenderContext renderContext, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.tabCount > 1) {
            ...
            if (this.tabSize == 0.0) {
                ...
                double selectionCenterY = tabStartY + this.tabWidth / 2.0
                        + (this.scrollHeight - this.tabWidth) * (double) this.selectedTabIndex / (this.tabCount - 1);

                GuiUtil.drawRect(this.right - this.tabWidth, selectionCenterY - this.tabWidth / 2.0, this.right, selectionCenterY + this.tabWidth / 2.0,
                        BUTTON_COLOR_ACTIVE);
            ...
            GuiUtil.drawQuad(arrowCenterX, this.top, this.right - this.tabWidth, this.top + this.tabWidth, this.right, this.top + this.tabWidth, arrowCenterX,
                    this.top, this.currentMouseLocation == MouseLocation.TOP_ARROW ? BUTTON_COLOR_FOCUS : BUTTON_COLOR_INACTIVE);

            GuiUtil.drawQuad(arrowCenterX, this.bottom, this.right, this.bottom - this.tabWidth, this.right - this.tabWidth, this.bottom - this.tabWidth,
                    arrowCenterX, this.bottom, this.currentMouseLocation == MouseLocation.BOTTOM_ARROW ? BUTTON_COLOR_FOCUS : BUTTON_COLOR_INACTIVE);
        }
        ...
        if (mouseX < this.left || mouseX > this.right || mouseY < this.top || mouseY > this.bottom) {
            ...
            if (mouseY <= this.top + this.tabWidth + this.itemSpacing / 2.0) {
                ...
                this.currentMouseIndex = MathHelper
                        .clamp((int) ((mouseY - this.top - this.tabWidth - this.itemSpacing / 2) / (this.scrollHeight) * this.tabCount), 0, this.tabCount - 1);
//                this.currentMouseIndex = (int) ((mouseX - this.left - this.tabWidth - this.actualItemMargin / 2) / (this.tabWidth + this.tabMargin));
            ...
            int newIndex = this.getFirstDisplayedIndex()
                    + (int) ((mouseY - this.top - this.itemSpacing / 2) / (this.actualItemSize + this.itemSpacing + this.captionHeight)) * this.columnsPerRow
