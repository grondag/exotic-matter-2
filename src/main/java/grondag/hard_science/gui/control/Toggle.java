...
public class Toggle extends GuiControl<Toggle> {
    ...
    protected void drawContent(IGuiRenderContext renderContext, int mouseX, int mouseY, float partialTicks) {
        ...
        GuiUtil.drawBoxRightBottom(this.left, this.targetAreaTop, boxRight, this.targetAreaBottom, 1,
                this.isMouseOver(mouseX, mouseY) ? BUTTON_COLOR_FOCUS : BUTTON_COLOR_ACTIVE);

        if (this.isOn) {
            GuiUtil.drawRect(this.left + 2, this.targetAreaTop + 2, boxRight - 2, this.targetAreaBottom - 2, BUTTON_COLOR_ACTIVE);
        }

        GuiUtil.drawAlignedStringNoShadow(renderContext.fontRenderer(), this.label, boxRight + CONTROL_INTERNAL_MARGIN, this.targetAreaTop, this.labelWidth,
                this.labelHeight, TEXT_COLOR_LABEL, HorizontalAlignment.LEFT, VerticalAlignment.MIDDLE);
    }
    ...
    protected boolean isMouseOver(int mouseX, int mouseY) {
        return !(mouseX < this.left || mouseX > this.left + this.labelHeight + CONTROL_INTERNAL_MARGIN + this.labelWidth || mouseY < this.targetAreaTop
                || mouseY > this.targetAreaBottom);
    }
