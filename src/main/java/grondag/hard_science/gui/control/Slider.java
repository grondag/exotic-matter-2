...
public class Slider extends GuiControl<Slider> {
    ...
    protected void drawContent(IGuiRenderContext renderContext, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.label != null && this.labelWidth > 0) {
            GuiUtil.drawAlignedStringNoShadow(renderContext.fontRenderer(), this.label, this.left, this.top, this.labelWidth, this.height, TEXT_COLOR_LABEL,
        ...
        GuiUtil.drawQuad(this.choiceRight, arrowCenterY, this.choiceRight + TAB_WIDTH, tabBottom, this.choiceRight + TAB_WIDTH, tabTop, this.choiceRight,
                arrowCenterY, this.currentMouseLocation == MouseLocation.LEFT_ARROW ? BUTTON_COLOR_FOCUS : BUTTON_COLOR_INACTIVE);

        GuiUtil.drawQuad(this.right, arrowCenterY, this.right - TAB_WIDTH, tabTop, this.right - TAB_WIDTH, tabBottom, this.right, arrowCenterY,
    ...
    private void updateMouseLocation(int mouseX, int mouseY) {
        ...
        if (mouseX < this.choiceRight || mouseX > this.right || mouseY < this.top || mouseY > this.top + TAB_WIDTH) {
            ...
            this.currentMouseIndex = MathHelper.clamp((int) ((mouseX - this.choiceRight - TAB_WIDTH - ITEM_SPACING / 2) / (this.scrollWidth) * this.size), 0,
                    this.size - 1);
        }
    }
    ...
    public void setSelectedIndex(int index) {
        this.selectedTabIndex = this.size == 0 ? NO_SELECTION : MathHelper.clamp(index, 0, this.size - 1);
    }
