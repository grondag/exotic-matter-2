...
public class ColorPicker extends GuiControl<ColorPicker> {
    ...
    private void changeHueIfDifferent(Hue newHue) {
        if (newHue != this.selectedHue) {
            ...
            ColorMap newMap = BlockColorMapProvider.INSTANCE.getColorMap(newHue, currentChroma, currentLuminance);

            while (newMap == null) {
                currentChroma = Chroma.VALUES[currentChroma.ordinal() - 1];
                newMap = BlockColorMapProvider.INSTANCE.getColorMap(newHue, currentChroma, currentLuminance);
            }
    ...
    protected void handleMouseClick(Minecraft mc, int mouseX, int mouseY, int clickedMouseButton) {
        ...
        if (distance < this.radiusOuter + 2) {
            ...
            if (l >= 0 && l < Luminance.COUNT && c >= 0 && c < Chroma.COUNT) {
                ColorMap testMap = BlockColorMapProvider.INSTANCE.getColorMap(this.selectedHue, Chroma.VALUES[c], Luminance.VALUES[l]);
