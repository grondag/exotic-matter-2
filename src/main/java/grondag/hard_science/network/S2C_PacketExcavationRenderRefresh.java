...
public abstract class S2C_PacketExcavationRenderRefresh
{
    ...
    public static Packet<?> toPacket(Collection<ExcavationRenderEntry> entries) {
        ...
        for(ExcavationRenderEntry r : entries)
        {
            ...
            if(positions != null)
            {
                if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id %d Refresh toBytes position count = %d", r.id, positions == null ? 0 : positions.length);
    ...
    public static void accept(PacketContext context, PacketByteBuf pBuff) {
        ...
        if(count > 0)
        {
            for(int i = 0; i < count; i++)
            {
                ...
                if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id %d Refresh toBytes position count = %d", id, list == null ? 0 : list.length);
