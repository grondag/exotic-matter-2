...
public interface SimpleModelState extends PrimitiveModelState<SimpleModelState, SimpleModelState.Mutable>  {
    ...
    static SimpleModelStateFunction MATCH_BLOCK = SimpleModelState.fromWorld(BlockTest.sameBlock());
    
    static SimpleModelStateFunction fromWorld(BlockTest<SimpleModelState> joinTest) {
        return () -> joinTest;
    }
    
    @FunctionalInterface
    public static interface SimpleModelStateFunction extends WorldToModelStateFunction {
        BlockTest<SimpleModelState> joinTest();
        
        @Override
        default void accept(ModelState.Mutable modelStateIn, XmBlockState xmBlockState, BlockView world, BlockPos pos, boolean refreshFromWorld) {
            if(refreshFromWorld) {
