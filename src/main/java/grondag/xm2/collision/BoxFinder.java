...
import static grondag.xm2.collision.BoxFinderUtils.EMPTY;
...
/**
 * Quickly identifies largest filled AABBs within an 8x8x8 voxel volume and
 * outputs those boxes plus any unclaimed voxels.
 * <p>
 * 
 * Exploits representation of each 8x8x1 layer as a single long value to enable
...
public class BoxFinder {
    ...
    /**
     * Tuples that have already been recursed and therefore don't need to be checked
     * again
     */
    ...
    public boolean isEmpty() {
        return voxels[0] == 0 && voxels[1] == 0 && voxels[2] == 0 && voxels[3] == 0 && voxels[4] == 0 && voxels[5] == 0
                && voxels[6] == 0 && voxels[7] == 0;
    }
    ...
    /**
     * Combines the two volume volumes that can be combined with the smallest number
     * of error voxels.
     */
    ...
    int simplificationScore(int volumeKey) {
        ...
        // If this volume would cause other maximal volume to split into two or more
        // volumes
        // that are not fully enclosed in this volume, then those count against the
        // score.
        int boxScore = -1;
    ...
    /**
     * If maximal volume at index does not intersect with any existing member, then
     * is added to the set and recursively attempt to add more members.
     * 
     * If the volume cannot be added to the set because it intersects, then this is
     * a leaf node - adds the volume as a disjoint set and returns false.
     * 
     */
