...
public class ExcavationRenderManager
{
    ...
    public static void render(float tickDelta, ClientPlayerEntity player)
    {
        ...
        for(ExcavationRenderer ex : renderCopy)
        {
            ...
            {
                if(ex.drawBounds(bufferbuilder, player, d0, d1, d2, (float) tickDelta)) secondPass.add(ex);
            }
        }
    ...
    public static void addOrUpdate(ExcavationRenderer... renders)
    {
        ...
        if(XmConfig.logExcavationRenderTracking) Xm.LOG.info("mass update, excavationSize = %d, renderSize = %d", excavations.size(), renderCopy.length);
    }
    ...
    public static void addOrUpdate(ExcavationRenderer render)
    {
        excavations.put(render.id, render);
        renderCopy = excavations.values().toArray(new ExcavationRenderer[excavations.size()]);
        if(XmConfig.logExcavationRenderTracking) Xm.LOG.info("addOrUpdate id = %d, excavationSize = %d, renderSize = %d", render.id, excavations.size(), renderCopy.length);
    }
    ...
    public static void remove(int id)
    {
        ...
        renderCopy = excavations.values().toArray(new ExcavationRenderer[excavations.size()]);
        if(XmConfig.logExcavationRenderTracking) Xm.LOG.info("remove id = %d, excavationSize = %d, renderSize = %d", id, excavations.size(), renderCopy.length);
    }
}
