...
import grondag.hard_science.network.S2C_PacketExcavationRenderRefresh;
...
public class ExcavationRenderTracker extends WorldMap<Int2ObjectOpenHashMap<ExcavationRenderEntry>>
{
    ...
    public void add(Job job)
    {
        ...
        if(XmConfig.logExcavationRenderTracking) Xm.LOG.info("id = %d new Entry, valid=%s", entry.id, Boolean.toString(entry.isValid()));
        if(entry.isValid())
        {
            synchronized(this)
            {
            ...
            for(Map.Entry<ServerPlayerEntity, PlayerData> playerEntry : playerTracking.entrySet())
            {
                ...
                {
                    if(XmConfig.logExcavationRenderTracking) Xm.LOG.info("adding listeners for %s", playerEntry.getKey().getDisplayName());
    ...
    {
        if(XmConfig.logExcavationRenderTracking) Xm.LOG.info("id = %d removing excavation render entry", entry.id);
    ...
    {
        if(XmConfig.logExcavationRenderTracking) Xm.LOG.info("updatePlayerTracking for %s", player.getName());
        ...
        {
            // no changes
            if(XmConfig.logExcavationRenderTracking) Xm.LOG.info("updatePlayerTracking exit no changes");
            return;
        }
            ...
            if(entries != null && !entries.isEmpty())
            {
                for(ExcavationRenderEntry entry : entries.values())
                {
                    if(entry.domainID == newData.domainID)
                    {
                        ...
                        if(entry.isFirstComputeDone()) output.add(entry);
                    }
                }
            }
        }
