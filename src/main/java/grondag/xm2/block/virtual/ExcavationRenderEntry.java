...
public class ExcavationRenderEntry implements ITaskListener, Runnable
{
    ...
    public ExcavationRenderEntry(Job job)
    {
        ...
        if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id = %d new Entry constructor", this.id);
        ...
        {
            if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id = %d new Entry constructor - invalid", this.id);
            this.isValid = false;
        }
        else
        {
            if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id = %d new Entry constructor - launching compute", this.id);
    ...
    {
        if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id = %d Compute called. Already running = %s", this.id, Boolean.toString(this.isScheduled.get()));
    ...
    public void run()
    {
        if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id = %d Compute running.", this.id);
        ...
        if(count == 0)
        {
            if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id = %d Compute existing due to empty positions.", this.id);
        ...
        if(count <= 16 && (this.renderPositions == null || this.renderPositions.length != count))
        {
            ...
            needsListenerUpdate = true;
            if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id %d Computed render position length = %d", this.id, this.renderPositions == null ? 0 : this.renderPositions.length);
        }
        
        if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id = %d Compute done, updateListeners=%s, isDirty=%s", this.id, Boolean.toString(needsListenerUpdate),
    ...
    {
        if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id=%d addListenger sendIfNew=%s, isValue=%s, isFirstComputeDone=%s",
                this.id,
        ...
        synchronized(this.listeners)
        {
            ...
            {
                if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id=%d addListenger scheduling packet.", this.id);
...
//        });
    ...
    {
        if(HsConfig.logExcavationRenderTracking) Xm.LOG.info("id %d Render position retrieval, count = %d", this.id, this.renderPositions == null ? 0 : this.renderPositions.length);
