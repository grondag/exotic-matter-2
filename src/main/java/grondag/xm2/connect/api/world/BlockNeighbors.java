...
import grondag.xm2.connect.impl.BlocksNeighborsImpl;
 ...
 * performant evaluation in uses cases that require results of more than one test.<p>
 * 
 * Instance methods are not thread-safe and this class is intended to be accessed
 * as re-used, threadlocal instances via {@link #threadLocal(BlockView, BlockPos)} and 
 * its variants. <p>
 * 
 * If thread local access is insufficient (for example, if two instances are needed
 * in the same scope) then {@link #claim(BlockView, BlockPos)} (or one of its variants)
 * can be used to retrieve or create a potentially pooled instance. If {@link #release()} is 
 * then called before the instance goes out of scope, it will be returned to a pool
 * for later re-used.<p>  
 * 
 * Both usage patterns (threadlocal and claim()/release()) will generally result in
 * no- or low- allocation.  Using these patterns may help prevent garbage-collection 
 * related lag spikes on clients when this utility is used during chunk rebuilds or 
 * other scenarios that result in high call volume.<p>
 *
 * Note that instances provided by {@link #claim(BlockView, BlockPos)} have no connection
 * to the re-use pool and no references are retained to them once claimed.  Calling
 * {@link #release()} is unnecessary and has no effect for instances returned by 
 * {@link #threadLocal(BlockView, BlockPos)}.
 */
@API(status = STABLE)
