...
public abstract class PlacementHandler {
     ...
     */
    public static PlacementResult doRightClickBlock(PlayerEntity player, @Nullable BlockPos onPos, @Nullable Direction onFace, @Nullable Vec3d hitVec, ItemStack stack, PlacementItem item)
    {
    ...
    public static int speciesForPlacement(PlayerEntity player, BlockPos onPos, Direction onFace, ItemStack stack, grondag.fermion.world.IBlockRegion region)
    {
        ...
        if(PlayerModifiers.isAltPressed(player)) mode = mode.alternate();
    ...
    {
        if(!player.canModifyWorld() || build == null || !build.isOpen()) return;
    ...
    private static boolean placeBlockAt(ItemStack stack, PlayerEntity player, World world, BlockPos pos, Direction side, float hitX, float hitY, float hitZ, BlockState newState)
    { 
