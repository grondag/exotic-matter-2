...
class StreamBackedMutablePolygon extends StreamBackedPolygon implements MutablePolygon {
	...
	public MutablePolygon copyInterpolatedVertexFrom(int targetIndex, Polygon from, int fromIndex, Polygon to, int toIndex, float toWeight) {
		...
		this.color(targetIndex, 0, ColorUtil.interpolate(from.color(fromIndex, 0), to.color(toIndex, 0), toWeight));
		uv(targetIndex, 0,
				Mth.lerp(toWeight, from.u(fromIndex, 0), to.u(toIndex, 0)),
				Mth.lerp(toWeight, from.v(fromIndex, 0), to.v(toIndex, 0)));

		if (layerCount > 1) {
			this.color(targetIndex, 1, ColorUtil.interpolate(from.color(fromIndex, 1), to.color(toIndex, 1), toWeight));
			uv(targetIndex, 1,
					Mth.lerp(toWeight, from.u(fromIndex, 1), to.u(toIndex, 1)),
					Mth.lerp(toWeight, from.v(fromIndex, 1), to.v(toIndex, 1)));

			if (layerCount == 3) {
				this.color(targetIndex, 2, ColorUtil.interpolate(from.color(fromIndex, 2), to.color(toIndex, 2), toWeight));
				uv(targetIndex, 2,
