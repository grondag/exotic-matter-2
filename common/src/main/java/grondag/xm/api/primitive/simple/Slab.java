...
public class Slab {
	...
	static final Function<PrimitiveState, XmMesh> POLY_FACTORY = modelState -> {
		...
		writer.setupFaceQuad(Direction.UP, 0, 0, 1, 1, 0.5f, Direction.NORTH);
		transform.accept(writer);
		writer.append();

		writer.surface(SURFACE_SIDES);
		writer.setupFaceQuad(Direction.EAST, 0, 0, 1, 0.5f, 0, Direction.UP);
		transform.accept(writer);
		writer.append();

		writer.surface(SURFACE_SIDES);
		writer.setupFaceQuad(Direction.WEST, 0, 0, 1, 0.5f, 0, Direction.UP);
		transform.accept(writer);
		writer.append();

		writer.surface(SURFACE_SIDES);
		writer.setupFaceQuad(Direction.NORTH, 0, 0, 1, 0.5f, 0, Direction.UP);
		transform.accept(writer);
		writer.append();

		writer.surface(SURFACE_SIDES);
		writer.setupFaceQuad(Direction.SOUTH, 0, 0, 1, 0.5f, 0, Direction.UP);
		transform.accept(writer);
		writer.append();
